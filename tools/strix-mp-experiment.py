#!/usr/bin/env python3
"""strix-mp-experiment — run multiple SHQ-T16 mixed-precision recipes and
measure retained quality + size, side-by-side.

For each preset in tools/strix/recipe.py: quantize, benchmark candidate-vs-
teacher (matched-token KL/ppl/top1), record artifact bytes + per-tier tensor
counts. Prints a comparison table.

Usage:
  strix-mp-experiment --source DIR --suite FILE --teacher-artifact DIR
...
def main(argv=None):
    ...
    if args.json:
        print(json.dumps({"schema": "strix.mp-experiment.v1", "results": results}, indent=2))
    else:
