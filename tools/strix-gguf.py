#!/usr/bin/env python3
"""strix-gguf — inspect and dequant-q4 GGUF model files.
...
you answer, for an unsloth quantization of the same model:
  ...
  * Dequant of Q4-family tensors back to f32/bf16, so their reconstruction
    error vs the bf16 source can be scored the same way strix-quantize.py
...
Usage:
  strix-gguf --gguf FILE [--card] [--types] [--tensors]
  strix-gguf --gguf FILE --dequant NAME                # print one tensor stats
  strix-gguf --gguf FILE --dequant-dir DIR --max N     # dump first N tensors to .npy
  strix-gguf --gguf FILE --recon --bf16-source SRC_MANIFEST_DIR \
...
# --------------------------------------------------------------------------
def main(argv=None):
    ap = argparse.ArgumentParser(prog="strix-gguf")
