...
from gufo.h3_quality import H3QualityError, audio_metrics, frame_metrics


SCHEMA = "gufo.minimax-h3-preset-quality.v1"
...
def sibling_parameters(path: Path) -> dict[str, Any] | None:
    ...
    if (
        ...
        or document.get("schema") != "gufo.minimax-h3-text-generation.v1"
    ):
...
def sibling_profile(
    ...
    if (
        not isinstance(document, dict)
        or document.get("schema") != "gufo.minimax-h3-profile-run.v1"
    ):
    ...
    *,
    ...
    return {
        ...
        "metric_implementations": {
            "windowed_ssim": {
                "name": "gufo-gaussian-windowed-ssim",
                "window_size": 11,
                "sigma": 1.5,
