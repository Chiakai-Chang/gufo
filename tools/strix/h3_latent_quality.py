...
SCHEMA = "strix.minimax-h3-latent-quality.v1"
...
def teacher_payloads(root: Path) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    ...
    if metadata.get("schema") != "strix.minimax-h3-denoiser-golden.v1":
...
def native_payloads(root: Path) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    ...
    if manifest.get("schema") != "strix.minimax-h3-final-latents.v1":
