...
| pp128 | 512 | 464 | 443 | — |
| pp256 | 759 | — | — | — |
| pp512 | 1013 | 796 | 628 | 402 |
| pp1024 | 1212 | 931 | — | — |
| pp2048 | 1335 (steady clock 1330–1340) | 1040 | 719 | 439 |
...
| DeltaNet recurrence operands staged through LDS | retained, 3.06 → 2.18 ms per layer (pp2048 1318 → 1335) | the serial token loop read q, k, v and the per-token scales straight from the 84 MB conv output, so each of the 2048 steps paid a DRAM latency (register prefetch of one token: 2.63 ms, 224 VGPRs); the block now stages four tokens per window into LDS with three windows of loads in flight, stored [key chunk][segment] so a row's four lanes hit adjacent banks (row-major was a four-way conflict: 1198 tok/s). What remains is the elementwise floor: eight rows per wave re-read the same q/k fragments (64 LDS cycles per wave-token across 40 waves per WGP) and about 150 instructions per token, so a chunked formulation on the matrix cores is the lever left |
| chunked DeltaNet on the matrix cores | rejected for now, 5.9 ms per layer against 2.18 (pp2048 1335 → 1240) | 32-token chunks, one block per value head: with G the cumulative decay the recurrence unrolls into (I + A) u = diag(β)(V − diag(G) K S₀ᵀ), A_ij = β_i (G_i/G_j) k_i·k_j, O = diag(G) Q S₀ᵀ + (M∘QKᵀ) u, S_C = G_C S₀ + Σ (G_C/G_i) u_i k_iᵀ; every product a 16x16x16 F16 WMMA (68 per wave per chunk), the state as F32 accumulator fragments (wave w owns 16 value rows, all keys), log-space decays (a zero decay resets the state: its log is floored at −80, else the ratios go NaN), the triangular solve in F32 on one wave. Correct: a numpy model with the same F16 roundings reproduces the kernel's error to three digits (worst rows 7.5% of their own RMS where the output cancels, 1.3% of a head's state; q and k rounding dominate; the F32 loop is at 1e-5), and the model validates unchanged (rmse 0.28). Slow because the compiler spilled the state fragments to scratch (256 VGPRs, 1 KB scratch: array captures in lambdas) and the K-transposed operands were 16 two-byte gathers per fragment (270 per chunk); 3,700 instructions and 600 waits per chunk, 92 µs each. The fixes are known (named accumulator fragments, K transposed once per chunk into the dead Q rows) and the F64 recurrence reference now in `gdn_ops_test` measures per-row errors for the next attempt |
...
- DeltaNet recurrence (2.2 ms per layer, 5% of a pass) is a sequential
  per-token chain at its elementwise instruction floor; the chunked WMMA
  formulation above is correct but spills — with the state fragments kept
  in registers and K transposed once per chunk it should land near 0.5 ms
  per layer (68 WMMAs per wave per 32-token chunk).
