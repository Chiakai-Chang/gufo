...
void TestMtpDraftBackend() {
  gufo::speculative::MtpDraftHeadConfig config;
  config.num_heads = 4;
  config.hidden_size = 256;
  config.vocab_size = 1000;

  gufo::speculative::MtpDraftBackend backend(config);
  ...
  assert(proposal.tokens ==
         std::vector<gufo::tokenization::TokenId>({30, 40, 50}));
  ...
  const std::vector<gufo::tokenization::TokenId> no_match = {42, 43, 44};
...
void TestSelfSpeculativeBackend() {
  gufo::speculative::SelfSpeculativeConfig config;
  config.total_layers = 32;
  config.exit_layer = 8;
  config.draft_step_count = 3;

  gufo::speculative::SelfSpeculativeBackend backend(config);
  assert(backend.Name() == "SelfSpeculativeBackend");

  const std::vector<gufo::tokenization::TokenId> prompt = {10, 20, 30};
