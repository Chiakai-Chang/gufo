...
void TestPromptLookupExactMatch() {
  gufo::speculative::PromptLookupConfig config;
  config.max_ngram_size = 3;
  config.min_ngram_size = 2;
  config.max_draft_tokens = 3;

  gufo::speculative::PromptLookupDraftBackend backend(config);
  ...
  std::vector<gufo::tokenization::TokenId> seq = {10, 20, 30, 40,
                                                  50, 99, 10, 20};
...
void TestPromptLookupNoMatchReturnsEmpty() {
  ...
  std::vector<gufo::tokenization::TokenId> seq = {1, 2, 3, 4, 5, 6, 7};
...
void TestPromptLookupShortSequence() {
  ...
  std::vector<gufo::tokenization::TokenId> seq = {10};
