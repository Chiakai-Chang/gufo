...
struct FakeState final : strix::server::ContinuationState {
...
void TestColdMissThenExactExtensionHit() {
  ...
  strix::server::ContinuationCache cache(1, [&] {
    return std::make_unique<FakeState>(next_id++, &invalidations);
  });

  const std::vector<strix::server::ContinuationToken> first_prompt{1, 2, 3};
  {
  ...
  const std::vector<strix::server::ContinuationToken> extension{1, 2, 3,
                                                                4, 5, 6};
  {
...
void TestDivergenceInvalidatesOldState() {
  std::vector<std::size_t> invalidations(1);
  strix::server::ContinuationCache cache(
      1, [&] { return std::make_unique<FakeState>(0, &invalidations); });

  {
    auto lease =
        cache.Acquire(std::vector<strix::server::ContinuationToken>{1, 2});
    lease.Commit({1, 2, 3});
  }
  {
    auto lease =
        cache.Acquire(std::vector<strix::server::ContinuationToken>{1, 9});
...
void TestUncommittedLeaseIsInvalidated() {
  ...
  {
    auto lease =
        cache.Acquire(std::vector<strix::server::ContinuationToken>{7});
  ...
  auto retry =
      cache.Acquire(std::vector<strix::server::ContinuationToken>{7, 8});
...
void TestLongestAvailablePrefixWins() {
  ...
  std::size_t next_id = 0;
  strix::server::ContinuationCache cache(2, [&] {
    return std::make_unique<FakeState>(next_id++, &invalidations);
  });

  {
    auto first =
        cache.Acquire(std::vector<strix::server::ContinuationToken>{1});
    first.Commit({1, 2});
  }
  {
    auto second =
        cache.Acquire(std::vector<strix::server::ContinuationToken>{9});
    second.Commit({9, 8, 7});
  }

  auto lease =
      cache.Acquire(std::vector<strix::server::ContinuationToken>{9, 8, 7, 6});
...
void TestWaitingAcquireCanBeCancelled() {
  ...
  strix::server::ContinuationCache cache(
      1, [&] { return std::make_unique<FakeState>(0, &invalidations); });
  auto held =
      cache.Acquire(std::vector<strix::server::ContinuationToken>{1, 2, 3});
  std::atomic<bool> cancelled{true};

  auto cancelled_lease =
      cache.Acquire(std::vector<strix::server::ContinuationToken>{1, 2, 3, 4},
