...
static __host__ mmvq_parameter_table_id get_device_table_id(int cc) {
    if (GGML_HIP_CC_IS_RDNA4(cc)) {
        return MMVQ_PARAMETERS_RDNA4;
    }
    if (GGML_HIP_CC_IS_RDNA3_0(cc)) {
        return MMVQ_PARAMETERS_RDNA3_0;
    }
    if (GGML_HIP_CC_IS_RDNA2(cc) || GGML_HIP_CC_IS_RDNA3_5(cc)) {
        return MMVQ_PARAMETERS_RDNA2;
    }
    if (GGML_HIP_CC_IS_GCN(cc) || GGML_HIP_CC_IS_CDNA(cc)) {
        return MMVQ_PARAMETERS_GCN;
    }
    return MMVQ_PARAMETERS_GENERIC;
}
...
int get_mmvq_mmid_max_batch(ggml_type type, int cc) {
    // AMD
    if (GGML_HIP_CC_IS_AMD(cc)) {
        if (GGML_HIP_CC_IS_RDNA4(cc)) {
            return get_mmvq_mmid_max_batch_rdna4(type);
        }
        if (GGML_HIP_CC_IS_RDNA3(cc)) {
            return get_mmvq_mmid_max_batch_rdna3(type);
        }
        if (GGML_HIP_CC_IS_RDNA1(cc) || GGML_HIP_CC_IS_RDNA2(cc)) {
            return get_mmvq_mmid_max_batch_rdna1_rdna2(type);
        }
        if (GGML_HIP_CC_IS_CDNA(cc)) {
            return get_mmvq_mmid_max_batch_cdna(type);
        }
        if (GGML_HIP_CC_IS_GCN(cc)) {
...
__launch_bounds__(calc_nwarps(type, ncols_dst, get_device_table_id())*ggml_hip_get_physical_warp_size(), 1)
static __global__ void mul_mat_vec_q(
        const void * __restrict__ vx, const void * __restrict__ vy, const int32_t * __restrict__ ids, const ggml_hip_mm_fusion_args_device fusion, float * __restrict__ dst,
...
#pragma unroll
            for (int i = 0; i < rows_per_hip_block; ++i) {
                tmp[j][i] += vec_dot_q_hip(
                    vx, &y[j*stride_col_y + kby], kbx_offset + i*stride_row_x + kbx, kqs);
                if constexpr (has_fusion) {
                    if (use_gate) {
                        tmp_gate[j][i] += vec_dot_q_hip(
                ...
                if (use_gate) {
                    ...
                    switch (active_glu) {
                        case GGML_GLU_OP_SWIGLU:
                            result *= ggml_hip_op_silu_single(gate_value);
                            break;
                        case GGML_GLU_OP_GEGLU:
                            result *= ggml_hip_op_gelu_single(gate_value);
                            break;
                        case GGML_GLU_OP_SWIGLU_OAI: {
                            result = ggml_hip_op_swiglu_oai_single(gate_value, result);
                            break;
                        }
                        default:
...
__launch_bounds__(get_mmvq_mmid_max_batch_for_device<type>()*ggml_hip_get_physical_warp_size(), 1)
...
static void mul_mat_vec_q_switch_fusion(
        const void * vx, const void * vy, const int32_t * ids, const ggml_hip_mm_fusion_args_device fusion, float * dst,
        const uint32_t ncols_x, const uint3 nchannels_y, const uint32_t stride_row_x, const uint32_t stride_col_y,
        const uint32_t stride_col_dst, const uint3 channel_ratio, const uint32_t stride_channel_x,
        const uint32_t stride_channel_y, const uint32_t stride_channel_dst, const uint3 sample_ratio,
        const uint32_t stride_sample_x, const uint32_t stride_sample_y, const uint32_t stride_sample_dst,
    ...
    if constexpr (type == GGML_TYPE_Q8_0 || c_ncols_dst == 1) {
        if (has_fusion) {
    ...
    GGML_ASSERT(!has_fusion && "multi-column fusion requires Q8_0");
...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    const int device = ggml_hip_get_device();
    const int                     cc        = ggml_hip_info().devices[device].cc;
    const int warp_size = ggml_hip_info().devices[device].warp_size;
    ...
    const auto should_use_small_k = [&](int c_ncols_dst) {
        ...
        if ((ncols_dst == 1 && std::find(iq_slow_other.begin(), iq_slow_other.end(), type) != iq_slow_other.end()) ||
            GGML_HIP_CC_IS_RDNA(cc)) use = false;

        return use;
    };

    if (has_ids) {
        // One kernel for every routed batch width keeps reduction rounding stable.
...
// The adapter calls the four supported quantized formats directly.
void mul_mat_vec_q_switch_type(
        const void * vx, const ggml_type type_x, const void * vy, const int32_t * ids, const ggml_hip_mm_fusion_args_device fusion, float * dst,
        const int ncols_x, const int nrows_x, const int ncols_dst,
        const int stride_row_x, const int stride_col_y, const int stride_col_dst,
        const int nchannels_x, const int nchannels_y, const int nchannels_dst,
        const int stride_channel_x, const int stride_channel_y, const int stride_channel_dst,
