...
// qfn_mmq.hip.cpp - host wrapper around llama.cpp's vendored mul_mat_q kernels.
//
// The fused target-prefill MoE dispatcher (qfn_mmq_fused_down,
// qfn_swiglu_weighted_f32, the fused_down branches of
// qfn_mmq_moe_pair_impl, and the qfn_mmq_iq2_xxs_q2_K_moe_fused_* entry
...
//
// Implements the public qfn_mmq_* entry points and explicitly instantiates
...
struct qfn_hip_persistent_scratch {
    void *ptr = nullptr;
    size_t bytes = 0;
    std::mutex mutex;
};

static qfn_hip_persistent_scratch g_hip_dense_q81_scratch;
static qfn_hip_persistent_scratch g_hip_pair_map_scratch;

static void *qfn_hip_persistent_scratch_reserve_locked(
        qfn_hip_persistent_scratch *scratch,
        size_t bytes,
        size_t granularity,
        cudaStream_t stream,
        const char *tag) {
...
extern "C" void qfn_mmq_set_aligned_q81_scratch(void *ptr, size_t bytes) {
...
// M2-Inc2a: registry of producer-emitted q8_1 activations (qfn_cuda.cu).
...
extern "C" int qfn_cuda_q8_fold_take_q81(const void *src, uint64_t in_dim,
                                         const void **q81);
static char *qfn_mmq_folded_q81(const float *X_f32, int64_t K, int n_tokens,
                                int64_t ne10_padded) {
    if (n_tokens != 1 || ne10_padded != K) return nullptr;
    const void *p = nullptr;
    if (!qfn_cuda_q8_fold_take_q81((const void *)X_f32, (uint64_t)K, &p)) return nullptr;
    static int logged = 0;
    if (!logged) {
        logged = 1;
...
extern "C" int qfn_mmq_init(int device) {
    ...
    if (info.device_count == 0) {
        fprintf(stderr, "qfn_mmq_init: no CUDA devices found\n");
        return -1;
    }
...
extern "C" void qfn_mmq_cleanup(void) {
#if defined(GGML_USE_HIP)
...
static bool qfn_should_use_mmq_impl(enum ggml_type type, int cc, int64_t ne11, int64_t n_experts) {
...
extern "C" int qfn_mmq_should_use(int type_x, int64_t ne11, int64_t n_experts) {
    const int dev = ggml_cuda_get_device();
    const int cc  = ggml_cuda_info().devices[dev].cc;
    const enum ggml_type t = (enum ggml_type) type_x;
    return qfn_should_use_mmq_impl(t, cc, ne11, n_experts) ? 1 : 0;
}
...
static void qfn_mmq_sanitize_f32(float *p, uint64_t n, cudaStream_t stream) {
    if (!p || n == 0) return;
...
bool qfn_mmq_k_tile_supported(const char *tag, int K, int cc) {
...
int qfn_mmq_dense_impl(
    ...
    if (K % 32 != 0) {
        // qwen38: the DeepSeek adapter required K % 256 == 0, but the
        // activation is zero-padded to MATRIX_ROW_PADDING and the k-quant
        // types demand their own 256 alignment below, so block-32 formats
        // only need K % 32 (Qwen3.8-Flash-Next down projections have K=640).
        // Device weight buffers must carry a tail margin for the over-read.
    ...
    const int dev = ggml_cuda_get_device();
    const int cc  = ggml_cuda_info().devices[dev].cc;
    if (!qfn_mmq_k_tile_supported<type>(tag, K, cc)) return -1;
    ...
    if (stream == (cudaStream_t)0) {
        ...
        src1_q8_1 = (char *)qfn_hip_persistent_scratch_reserve_locked(
...
extern "C" int qfn_mmq_q8_0_dense_preq(
        const void * W, const void * Y_q8_mmq, size_t y_bytes, float * out,
        int M, int N, int K, cudaStream_t stream) {
    const char *tag = "qfn_mmq_q8_0_dense_preq";
    if (!W || !Y_q8_mmq || !out) {
        fprintf(stderr, "%s: null pointer\n", tag);
        return -1;
    }
    ...
    if (!inline_sanitize) {
        qfn_mmq_sanitize_f32(out, (uint64_t)M * (uint64_t)N, stream);
    }
    return 0;
}

// Dense Q8_0 D2R entry: same activation quantize + scratch treatment as
// qfn_mmq_dense_impl (incl. the S1.1a zero for the never-written tail), then
...
// through an isfinite guard.  Caller (qfn_cuda.cu) resolves W_aligned and
// gates on shape (M%128, K%1024, K<=4096) + n_tok.
extern "C" int qfn_mmq_q8_0_dense_d2r(
        const void * W_aligned, const float * X_f32, float * out_f32,
        int M, int N, int K, cudaStream_t stream) {
    const char *tag = "qfn_mmq_q8_0_dense_d2r";
    ...
    const int dev = ggml_cuda_get_device();
    const int cc  = ggml_cuda_info().devices[dev].cc;
    if (!qfn_mmq_q8_0_dense_d2r_available(cc)) {
        return -1;
    }
    ggml_backend_cuda_context * ctx = get_ctx_for_device(dev);
    if (!ctx) {
    ...
    return qfn_mmq_q8_0_dense_d2r_launch(W_aligned, src1_q8_1.get(), out_f32,
                                         M, N, K, stream);
}
 ...
 * qfn_mmq_q8_0_dense_d2r minus the activation quantize; the caller's
...
extern "C" int qfn_mmq_q8_0_dense_d2r_preq(
    ...
    cudaMemsetAsync((char *)Y_q8_mmq + data_bytes, 0, slack_bytes, stream);
    return qfn_mmq_q8_0_dense_d2r_launch(W_aligned, Y_q8_mmq, out_f32,
                                         M, N, K, stream);
}

extern "C" int qfn_mmq_q2_K_dense(
        const void * W, const float * X, float * out,
        int M, int N, int K, cudaStream_t stream) {
    return qfn_mmq_dense_impl<GGML_TYPE_Q2_K>("qfn_mmq_q2_K_dense", W, X, out, M, N, K, stream);
}

extern "C" int qfn_mmq_iq2_xxs_dense(
        const void * W, const float * X, float * out,
        int M, int N, int K, cudaStream_t stream) {
    return qfn_mmq_dense_impl<GGML_TYPE_IQ2_XXS>("qfn_mmq_iq2_xxs_dense", W, X, out, M, N, K, stream);
}

extern "C" int qfn_mmq_q4_K_dense(
        const void * W, const float * X, float * out,
        int M, int N, int K, cudaStream_t stream) {
    return qfn_mmq_dense_impl<GGML_TYPE_Q4_K>("qfn_mmq_q4_K_dense", W, X, out, M, N, K, stream);
}

extern "C" int qfn_mmq_mxfp4_dense(
        const void * W, const float * X, float * out,
        int M, int N, int K, cudaStream_t stream) {
    return qfn_mmq_dense_impl<GGML_TYPE_MXFP4>("qfn_mmq_mxfp4_dense", W, X, out, M, N, K, stream);
}
...
int qfn_mmq_moe_impl(
    ...
    qfn_pool_set_stream(stream);  /* task #22: pool ops must be stream-ordered with the kernels (see qfn_mmq_dense_impl) */
    ...
    if (type == GGML_TYPE_Q2_K && x_soa != nullptr &&
        ...
        if (d2r_avail_cc != cc) {
            d2r_avail_cc = cc;
            d2r_avail = qfn_mmq_q2_K_moe_d2r_available(cc) ? 1 : 0;
        }
        if (d2r_avail) {
            const size_t d2r_work_bytes =
                qfn_mmq_q2_K_moe_d2r_scratch_bytes(ne_get_rows, n_experts);
            if (d2r_work_bytes != 0) {
                ggml_cuda_pool_alloc<char> d2r_work(ctx->pool(), d2r_work_bytes);
                const int d2r_rc = qfn_mmq_q2_K_moe_d2r_launch(
    ...
    /* qwen38: the generic routed entry honors the same launch-geometry hints
     * as the fused DeepSeek path (qfn_mmq_set_routed_max_expert_rows /
     * qfn_mmq_set_routed_tile_cols). Without them a 512-token chunk over 512
     * experts launches 64 column tiles per expert for a mean bucket of ten
     * rows, and nearly every block exits at its first instruction. */
    args.ncols_grid_max =
    ...
    if (sanitize_out) {
        qfn_mmq_sanitize_f32(out_f32, (uint64_t)M * (uint64_t)ne_get_rows, stream);
    }
    return 0;
}

struct qfn_mmq_fused_down {
    const void  * W;
    const char  * W_soa;
    int64_t       soa_blocks;
     ...
     * fused_direct_soa doc in qfn_mmq.h); NULL = quantize internally. */
    const void  * input_q8_ext;
    size_t        input_q8_ext_bytes;
};

struct qfn_mmq_swiglu_epilogue {
    const float * router_weights;
    float       * mid_f32;
    half        * mid_f16;
    float         clamp;
...
static bool qfn_mmq_take_scratch(
...
static bool qfn_mmq_scratch_overlaps(
...
static __global__ void qfn_swiglu_weighted_f32(
...
// header comment on qfn_mmq_iq2_xxs_moe_pair for motivation.  Internal
// structure mirrors qfn_mmq_moe_impl above; the only differences are the
...
int qfn_mmq_moe_pair_impl(
     ...
     * shape, so one block count); see qfn_mmq_moe_impl. */
    const char* xa_soa = NULL, const char* xb_soa = NULL,
    int64_t soa_blocks = 0,
    /* ds4 (P3): see qfn_mmq_moe_impl. */
    bool sanitize_out = true, const qfn_mmq_fused_down* fused_down = nullptr,
    const qfn_mmq_swiglu_epilogue* swiglu_epilogue = nullptr) {
  ...
  const int dev = ggml_cuda_get_device();
  const int cc = ggml_cuda_info().devices[dev].cc;
  if (!qfn_mmq_k_tile_supported<type>(tag, K, cc))
    return -1;
  ...
  qfn_pool_set_stream(stream); /* task #22: pool ops must be stream-ordered with
                                  the kernels (see qfn_mmq_dense_impl) */
  ...
  int32_t* expert_bounds = nullptr;
    ...
    if (direct_gateup_q8) {
        ...
        const size_t gateup_work_bytes =
            qfn_mmq_iq2_xxs_moe_d2r_fused_scratch_bytes(
        ...
        if (!qfn_mmq_iq2_xxs_moe_d2r_available(cc) ||
            !qfn_mmq_q2_K_moe_d2r_available(cc)) {
            return -96;
        }
        ...
        if (stream == (cudaStream_t)0 &&
            ...
            int32_t *map = (int32_t *)
                qfn_hip_persistent_scratch_reserve_locked(
    ...
    // Same cap guard as qfn_mmq_moe_impl (see comment there): past the smem
    // cap the launcher takes the bit-identical global variant (P5).
    cudaError_t err = cudaSuccess;
    {
      // Zero the ID maps, as in qfn_mmq_moe_impl
     ...
     * qfn_mmq_set_routed_max_expert_rows). ncols_max keeps selecting the tile
    ...
    if (direct_gateup_q8) {
        ...
        {
          const int d2r_rc = qfn_mmq_iq2_xxs_moe_d2r_fused_launch(
        ...
        {
          const int down_rc = qfn_mmq_q2_K_moe_d2r_launch(
    ...
    if (!swiglu_epilogue &&
        ...
        if (d2r_iq2_avail_cc != cc) {
            d2r_iq2_avail_cc = cc;
            d2r_iq2_avail = qfn_mmq_iq2_xxs_moe_d2r_available(cc) ? 1 : 0;
        }
        if (d2r_iq2_avail) {
            const size_t d2r_work_bytes =
                qfn_mmq_iq2_xxs_moe_d2r_pair_scratch_bytes(ne_get_rows, n_experts);
            if (d2r_work_bytes != 0) {
                ggml_cuda_pool_alloc<char> d2r_work(ctx->pool(), d2r_work_bytes);

                const int d2r_rc = qfn_mmq_iq2_xxs_moe_d2r_pair_launch(
    ...
    if (fused_down) {
        ...
        {
          qfn_swiglu_weighted_f32<<<(uint32_t)((mid_values + 255u) / 256u), 256,
                                    0, stream>>>(
        ...
        if (fused_down->W_soa != nullptr &&
            ...
            if (work_bytes != 0u) {
                ...
                down_done = qfn_mmq_q2_K_moe_d2r_launch(
                        fused_down->W_soa,
    ...
    if (sanitize_out && !swiglu_epilogue) {
        qfn_mmq_sanitize_f32(out_a, (uint64_t)M * (uint64_t)ne_get_rows, stream);
        qfn_mmq_sanitize_f32(out_b, (uint64_t)M * (uint64_t)ne_get_rows, stream);
    }
    return 0;
}
...
/* qwen38: the routed projection without the trailing sanitize pass. */
extern "C" int qfn_mmq_q8_0_moe_raw(
    ...
    return qfn_mmq_moe_impl<GGML_TYPE_Q8_0>("qfn_mmq_q8_0_moe_raw", W, X, ids, out, M, K,
                                            ...
                                            nullptr, 0, /*sanitize_out=*/false);
}

extern "C" int qfn_mmq_q8_0_moe(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_impl<GGML_TYPE_Q8_0>("qfn_mmq_q8_0_moe", W, X, ids, out, M, K,
                                            n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_q2_K_moe(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_impl<GGML_TYPE_Q2_K>("qfn_mmq_q2_K_moe", W, X, ids, out, M, K,
                                            n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_iq2_xxs_moe(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_impl<GGML_TYPE_IQ2_XXS>("qfn_mmq_iq2_xxs_moe", W, X, ids, out, M, K,
                                               n_tokens, n_experts, n_expert_used, stream);
}
...
extern "C" int qfn_mmq_q2_K_moe_soa(
    ...
    return qfn_mmq_moe_impl<GGML_TYPE_Q2_K>("qfn_mmq_q2_K_moe_soa", W_soa, X, ids, out, M, K,
                                            n_tokens, n_experts, n_expert_used, stream,
...
extern "C" int qfn_mmq_q4_K_moe(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_impl<GGML_TYPE_Q4_K>("qfn_mmq_q4_K_moe", W, X, ids, out, M, K,
                                            n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_mxfp4_moe(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_impl<GGML_TYPE_MXFP4>("qfn_mmq_mxfp4_moe", W, X, ids, out, M, K,
                                             n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_iq2_xxs_moe_pair(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_pair_impl<GGML_TYPE_IQ2_XXS>(
        "qfn_mmq_iq2_xxs_moe_pair", W_a, W_b, X, ids, out_a, out_b,
        M, K, n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_iq2_xxs_moe_pair_token_bound(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
  return qfn_mmq_moe_pair_impl<GGML_TYPE_IQ2_XXS, true>(
      "qfn_mmq_iq2_xxs_moe_pair_token_bound", W_a, W_b, X, ids, out_a, out_b, M,
      K, n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_iq2_xxs_moe_pair_token_bound_swiglu(
    ...
    const qfn_mmq_swiglu_epilogue epilogue = {
        router_weights,
        mid_f32,
        (half *)mid_f16,
        clamp,
    ...
    return qfn_mmq_moe_pair_impl<GGML_TYPE_IQ2_XXS, true>(
        "qfn_mmq_iq2_xxs_moe_pair_token_bound_swiglu", W_gate, W_up, X, ids,
...
extern "C" int qfn_mmq_iq2_xxs_moe_pair_soa(
        const void * Wa_soa, const void * Wb_soa,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    if (M <= 0 || K <= 0 || K % 256 != 0 || n_experts <= 0) {
        fprintf(stderr, "qfn_mmq_iq2_xxs_moe_pair_soa: bad shape M=%d K=%d nexp=%d\n", M, K, n_experts);
        return -1;
    }
    const int64_t nblk = (int64_t)n_experts * (int64_t)M * (int64_t)(K/256);
    /* sanitize_out=false: see qfn_mmq_q2_K_moe_soa. */
    return qfn_mmq_moe_pair_impl<GGML_TYPE_IQ2_XXS>(
        "qfn_mmq_iq2_xxs_moe_pair_soa", Wa_soa, Wb_soa, X, ids, out_a, out_b,
        M, K, n_tokens, n_experts, n_expert_used, stream,
...
extern "C" int qfn_mmq_iq2_xxs_q2_K_moe_fused_soa(
    ...
    const qfn_mmq_fused_down fused_down = {
        W_down,
        (const char *)W_down,
        q2_pairs,
        router_weights,
    ...
    return qfn_mmq_moe_pair_impl<GGML_TYPE_IQ2_XXS>(
        "qfn_mmq_iq2_xxs_q2_K_moe_fused_soa", W_gate, W_up, X, ids, gate, up,
...
extern "C" int qfn_mmq_iq2_xxs_q2_K_moe_fused_direct_soa(
        const void * W_gate, const void * W_up, const void * W_down,
        const float * X, const int32_t * ids, const float * router_weights,
    ...
    return qfn_mmq_moe_pair_impl<GGML_TYPE_IQ2_XXS>(
        "qfn_mmq_iq2_xxs_q2_K_moe_fused_direct_soa", W_gate, W_up, X, ids,
...
extern "C" int qfn_mmq_q4_K_moe_pair(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_pair_impl<GGML_TYPE_Q4_K>(
        "qfn_mmq_q4_K_moe_pair", W_a, W_b, X, ids, out_a, out_b,
        M, K, n_tokens, n_experts, n_expert_used, stream);
}

/* qwen38: gate/up over a true top-k routing (no expert repeats per token,
 * so buckets are bounded by n_tokens) with the whole-buffer sanitize pass
 * skipped; every routed slot is written and consumed as-is. */
extern "C" int qfn_mmq_q4_K_moe_pair_unique(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_pair_impl<GGML_TYPE_Q4_K, true>(
        "qfn_mmq_q4_K_moe_pair_unique", W_a, W_b, X, ids, out_a, out_b,
        M, K, n_tokens, n_experts, n_expert_used, stream,
        nullptr, nullptr, 0, /*sanitize_out=*/false);
}

extern "C" int qfn_mmq_mxfp4_moe_pair(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_pair_impl<GGML_TYPE_MXFP4>(
        "qfn_mmq_mxfp4_moe_pair", W_a, W_b, X, ids, out_a, out_b,
        M, K, n_tokens, n_experts, n_expert_used, stream);
}
...
//
// Layout contract (see qfn_mmq.h): W_aligned = [__half dq[nblk]][pad to 64B]
...
// qfn_mmq_moe_gate_up_mid_q8_1_qwarp32_kernel) -> mid directly.  Replaces
...
template <ggml_type type>
int qfn_mmq_moe_pair_raw_vec_impl(
        const char    * tag,
        const void    * W_a,
        const void    * W_b,
        const float   * X_f32,
    ...
    qfn_mmq_sanitize_f32(out_a, out_count, stream);
    qfn_mmq_sanitize_f32(out_b, out_count, stream);
    return 0;
}

template <ggml_type type>
int qfn_mmq_moe_pair_vec_impl(
        const char    * tag,
        const void    * W_a,
        const void    * W_b,
        const float   * X_f32,
    ...
    // for graph capture compatibility.  See qfn_mmq_moe_vec_impl.
    qfn_pool_set_stream(stream);
    ...
    qfn_mmq_sanitize_f32(out_silu, (uint64_t)M * (uint64_t)n_expert_used, stream);
    return 0;
}

template <ggml_type type>
int qfn_mmq_dense_vec_impl(
        const char  * tag,
        const void  * W,
        const float * X_f32,
        float       * out_f32,
...
template <ggml_type type> struct qfn_mmq_vdr_mmvq_value;
template <> struct qfn_mmq_vdr_mmvq_value<GGML_TYPE_IQ2_XXS> { static constexpr int value = VDR_IQ2_XXS_Q8_1_MMVQ; };
template <> struct qfn_mmq_vdr_mmvq_value<GGML_TYPE_Q2_K>    { static constexpr int value = VDR_Q2_K_Q8_1_MMVQ; };
template <> struct qfn_mmq_vdr_mmvq_value<GGML_TYPE_Q4_K>    { static constexpr int value = VDR_Q4_K_Q8_1_MMVQ; };
template <> struct qfn_mmq_vdr_mmvq_value<GGML_TYPE_MXFP4>   { static constexpr int value = VDR_MXFP4_Q8_1_MMVQ; };

template <ggml_type type>
static __device__ __forceinline__ float qfn_mmq_vec_dot_q8_1(
...
static __device__ __forceinline__ float qfn_mmq_half_warp_sum_f32(float v) {
...
template <ggml_type type>
static __global__ void qfn_mmq_moe_down_sum6_q8_1_qwarp32_kernel(
        const void       * __restrict__ W,
        const block_q8_1 * __restrict__ X_q8,
    ...
    constexpr int vdr = qfn_mmq_vdr_mmvq_value<type>::value;
...
#pragma unroll
        for (uint32_t slot = 0; slot < top_k; ++slot) {
            ...
            for (uint32_t b = kbx0; !invalid_id && b < blocks_per_row_x; b += blocks_per_iter) {
                acc += qfn_mmq_vec_dot_q8_1<type>(
                    W, xq + (uint64_t)b * q8_per_k, kbx_base + (int)b, kqs);
            }
            acc = qfn_mmq_half_warp_sum_f32(acc);
            if (lane == 0) {
                if (!isfinite(acc)) acc = 0.0f;
                total += acc;
            }
...
template <ggml_type type>
static __global__ void qfn_mmq_moe_gate_up_mid_q8_1_qwarp32_kernel(
...
#pragma unroll
    for (uint32_t rr = 0; rr < 4u; ++rr) {
        ...
        for (uint32_t b = kbx0; !invalid_id && b < blocks_per_row_x; b += blocks_per_iter) {
            ...
            gate += qfn_mmq_vec_dot_q8_1<type>(W_gate, xb, kbx, kqs);
            up   += qfn_mmq_vec_dot_q8_1<type>(W_up,   xb, kbx, kqs);
        }
        gate = qfn_mmq_half_warp_sum_f32(gate);
        up   = qfn_mmq_half_warp_sum_f32(up);
        if (lane == 0) {
            if (!isfinite(gate)) gate = 0.0f;
            if (!isfinite(up)) up = 0.0f;
            if (clamp > 1.0e-6f) {
...
static __global__ void qfn_mmq_moe_down_sum6_vec_kernel(
        const void       * __restrict__ W,
        const block_q8_1 * __restrict__ X_q8,
        const int32_t    * __restrict__ ids,
        float            * __restrict__ out,
...
#pragma unroll
        for (int i = 0; i < c_rows_per_block; ++i) {
            tmp[i] += qfn_mmq_vec_dot_q8_1<type>(
...
template <ggml_type type>
int qfn_mmq_moe_down_sum6_vec_impl(
        const char    * tag,
        const void    * W,
        const float   * X_f32,
        const int32_t * ids,
    ...
    qfn_mmq_moe_down_sum6_q8_1_qwarp32_kernel<type><<<block_nums, block_dims, 0, stream>>>(
...
template <ggml_type type, int c_rows_per_block>
static __global__ void qfn_mmq_moe_gate_up_mid_vec_kernel(
        const void       * __restrict__ W_gate,
        const void       * __restrict__ W_up,
        const block_q8_1 * __restrict__ X_q8,
        const int32_t    * __restrict__ ids,
...
#pragma unroll
        for (int i = 0; i < c_rows_per_block; ++i) {
            ...
            gate[i] += qfn_mmq_vec_dot_q8_1<type>(W_gate, &y[kby], row_kbx, kqs);
            up[i]   += qfn_mmq_vec_dot_q8_1<type>(W_up,   &y[kby], row_kbx, kqs);
        }
    }
...
template <ggml_type type, int c_rows_per_block>
static __global__ void qfn_mmq_moe_gate_up_mid_vec_by_slot_kernel(
        const void       * __restrict__ W_gate,
        const void       * __restrict__ W_up,
        const block_q8_1 * __restrict__ X_q8,
        const int32_t    * __restrict__ ids,
...
template <ggml_type type>
int qfn_mmq_moe_gate_up_mid_vec_impl(
        const char    * tag,
        const void    * W_gate,
        const void    * W_up,
        const float   * X_f32,
    ...
    char *src1_q8_1_ptr = qfn_mmq_folded_q81(X_f32, K, n_tokens, ne10_padded);
    cudaError_t err;
    if (!src1_q8_1_ptr) {
        src1_q8_1_pool.alloc(ctx->pool(), nbytes_q8_1);
        src1_q8_1_ptr = src1_q8_1_pool.get();
    ...
    const dim3 block_dims(256);
    qfn_mmq_moe_gate_up_mid_q8_1_qwarp32_kernel<type><<<block_nums, block_dims, 0, stream>>>(
...
extern "C" int qfn_mmq_q2_K_moe_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_vec_impl<GGML_TYPE_Q2_K>(
        "qfn_mmq_q2_K_moe_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_iq2_xxs_moe_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_vec_impl<GGML_TYPE_IQ2_XXS>(
        "qfn_mmq_iq2_xxs_moe_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}
...
extern "C" int qfn_mmq_mxfp4_moe_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_vec_impl<GGML_TYPE_MXFP4>(
        "qfn_mmq_mxfp4_moe_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

// M1-Inc2b: exact inverse of the weight-server repack
// (repack_iq2_xxs_aligned_kernel, tools/qfn_weight_server.cu): aligned-SoA
...
extern "C" int qfn_mmq_iq2_xxs_aligned_derepack(
        const void * W_aligned, void * raw_out,
        int M, int K, int n_experts, cudaStream_t stream) {
    const char *tag = "qfn_mmq_iq2_xxs_aligned_derepack";
...
extern "C" uint64_t qfn_mmq_q8_0_aligned_bytes(int M, int K) {
...
extern "C" int qfn_mmq_q8_0_aligned_dense_vec(
        const void * W_aligned, const float * X_f32, float * out_f32,
        int M, int N, int K, cudaStream_t stream) {
    const char *tag = "qfn_mmq_q8_0_aligned_dense_vec";
    if (!W_aligned || !X_f32 || !out_f32) {
        fprintf(stderr, "%s: null pointer\n", tag);
        return -1;
    }
    ...
    char *x8 = N == 1 ? qfn_mmq_folded_q81(X_f32, K, 1, ne10_padded) : NULL;
    cudaError_t err;
    if (!x8) {
...
// --repack-q2k-aligned, DERIVED_Q2_K_ALIGNED_MOE, and qfn_mmq.h):
//
...
// tools/qfn_weight_server.cu): pair-SoA -> raw block_q2_K byte stream.  One
...
extern "C" uint64_t qfn_mmq_q2_k_aligned_bytes(int M, int K, int n_experts) {
...
extern "C" int qfn_mmq_q2_K_aligned_moe_vec(
    ...
    const char *tag = "qfn_mmq_q2_K_aligned_moe_vec";
    ...
    /* Quantize verbatim from qfn_mmq_moe_vec_impl<GGML_TYPE_Q2_K> so the q8_1
    ...
    qfn_mmq_sanitize_f32(out_f32, (uint64_t)M * (uint64_t)n_tokens, stream);
    return 0;
}

extern "C" int qfn_mmq_q2_K_aligned_derepack(
        const void * W_aligned, void * raw_out,
        int M, int K, int n_experts, cudaStream_t stream) {
    const char *tag = "qfn_mmq_q2_K_aligned_derepack";
    if (!W_aligned || !raw_out) {
        fprintf(stderr, "%s: null pointer\n", tag);
        return -1;
    }
...
extern "C" uint64_t qfn_mmq_iq2_xxs_aligned_bytes(int M, int K, int n_experts) {
...
static char *iq2_aligned_quantize_xn(
    ...
    char *folded = qfn_mmq_folded_q81(X_f32, K, n_tokens, ne10_padded);
    if (folded) {
        return folded;
    }
    char *ptr = nullptr;
...
extern "C" int qfn_mmq_iq2_xxs_aligned_moe_gate_up_mid_vec(
        const void * W_gate_aligned, const void * W_up_aligned,
        const float * X_f32, const int32_t * ids, const float * weights,
        float * mid_f32,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        float clamp, cudaStream_t stream) {
    const char *tag = "qfn_mmq_iq2_xxs_aligned_moe_gate_up_mid_vec";
...
extern "C" int qfn_mmq_iq2_xxs_aligned_moe_vec(
        const void * W_aligned, const float * X_f32, const int32_t * ids, float * out_f32,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    const char *tag = "qfn_mmq_iq2_xxs_aligned_moe_vec";
    if (!W_aligned || !X_f32 || !ids || !out_f32) {
        fprintf(stderr, "%s: null pointer\n", tag);
        return -1;
    }
    ...
    // Quantize X into canonical Q8_1, exactly as qfn_mmq_moe_vec_impl does, so
    ...
    qfn_mmq_sanitize_f32(out_f32, (uint64_t)n_tokens * (uint64_t)M * (uint64_t)n_expert_used, stream);
    return 0;
}

extern "C" int qfn_mmq_q2_K_moe_down_sum6_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_down_sum6_vec_impl<GGML_TYPE_Q2_K>(
        "qfn_mmq_q2_K_moe_down_sum6_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_q4_K_moe_down_sum6_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_down_sum6_vec_impl<GGML_TYPE_Q4_K>(
        "qfn_mmq_q4_K_moe_down_sum6_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_mxfp4_moe_down_sum6_vec(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_down_sum6_vec_impl<GGML_TYPE_MXFP4>(
        "qfn_mmq_mxfp4_moe_down_sum6_vec", W, X, ids, out, M, K,
        n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_iq2_xxs_moe_gate_up_mid_vec(
        const void * W_gate, const void * W_up,
        const float * X, const int32_t * ids, const float * weights, float * mid,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        float clamp, cudaStream_t stream) {
    return qfn_mmq_moe_gate_up_mid_vec_impl<GGML_TYPE_IQ2_XXS>(
        "qfn_mmq_iq2_xxs_moe_gate_up_mid_vec", W_gate, W_up, X, ids, weights, mid,
        M, K, n_tokens, n_experts, n_expert_used, clamp, stream);
}

extern "C" int qfn_mmq_q4_K_moe_gate_up_mid_vec(
        const void * W_gate, const void * W_up,
        const float * X, const int32_t * ids, const float * weights, float * mid,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        float clamp, cudaStream_t stream) {
    return qfn_mmq_moe_gate_up_mid_vec_impl<GGML_TYPE_Q4_K>(
        "qfn_mmq_q4_K_moe_gate_up_mid_vec", W_gate, W_up, X, ids, weights, mid,
        M, K, n_tokens, n_experts, n_expert_used, clamp, stream);
}

extern "C" int qfn_mmq_mxfp4_moe_gate_up_mid_vec(
        const void * W_gate, const void * W_up,
        const float * X, const int32_t * ids, const float * weights, float * mid,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        float clamp, cudaStream_t stream) {
    return qfn_mmq_moe_gate_up_mid_vec_impl<GGML_TYPE_MXFP4>(
        "qfn_mmq_mxfp4_moe_gate_up_mid_vec", W_gate, W_up, X, ids, weights, mid,
        M, K, n_tokens, n_experts, n_expert_used, clamp, stream);
}

extern "C" int qfn_mmq_iq2_xxs_moe_pair_vec(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_silu,
        int M, int K, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_pair_vec_impl<GGML_TYPE_IQ2_XXS>(
        "qfn_mmq_iq2_xxs_moe_pair_vec", W_a, W_b, X, ids, out_silu,
        M, K, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_q4_K_moe_pair_vec(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_silu,
        int M, int K, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_pair_vec_impl<GGML_TYPE_Q4_K>(
        "qfn_mmq_q4_K_moe_pair_vec", W_a, W_b, X, ids, out_silu,
        M, K, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_iq2_xxs_moe_pair_raw_vec(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_pair_raw_vec_impl<GGML_TYPE_IQ2_XXS>(
        "qfn_mmq_iq2_xxs_moe_pair_raw_vec", W_a, W_b, X, ids, out_a, out_b,
        M, K, n_tokens, n_experts, n_expert_used, stream);
}

extern "C" int qfn_mmq_q4_K_moe_pair_raw_vec(
        const void * W_a, const void * W_b,
        const float * X, const int32_t * ids, float * out_a, float * out_b,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_pair_raw_vec_impl<GGML_TYPE_Q4_K>(
        "qfn_mmq_q4_K_moe_pair_raw_vec", W_a, W_b, X, ids, out_a, out_b,
        M, K, n_tokens, n_experts, n_expert_used, stream);
}
...
extern "C" int qfn_mmq_q8_0_dense_vec(
        const void * W, const float * X, float * out,
        int M, int N, int K, cudaStream_t stream) {
    return qfn_mmq_dense_vec_impl<GGML_TYPE_Q8_0>(
        "qfn_mmq_q8_0_dense_vec", W, X, out, M, N, K, stream);
}


// qwen38: block-32 Q5_1 and super-block Q5_K experts of the unsloth
// UD-Q4_K_XL artifact, plus Q5_1/Q5_K dense entries for completeness.
/* qwen38: the routed projection without the trailing sanitize pass. */
...
extern "C" int qfn_mmq_q5_1_moe(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_impl<GGML_TYPE_Q5_1>("qfn_mmq_q5_1_moe", W, X, ids, out, M, K,
                                            n_tokens, n_experts, n_expert_used, stream);
}
...
extern "C" int qfn_mmq_q5_K_moe(
        const void * W, const float * X, const int32_t * ids, float * out,
        int M, int K, int n_tokens, int n_experts, int n_expert_used,
        cudaStream_t stream) {
    return qfn_mmq_moe_impl<GGML_TYPE_Q5_K>("qfn_mmq_q5_K_moe", W, X, ids, out, M, K,
                                            n_tokens, n_experts, n_expert_used, stream);
}
