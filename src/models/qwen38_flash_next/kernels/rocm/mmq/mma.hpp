#pragma once
// HIP matrix fragments and AMD WMMA/MFMA operations.
// Fragment coordinates are measured in physical 32-bit elements.
// Shared-memory and generic loads require 16-byte aligned pointers.

#include "common.hpp"


static __device__ __forceinline__ int ggml_hip_movmatrix(const int x) {
    ...
    const int ret_low  = (__shfl(x, src_laneid_low,  WARP_SIZE) >> shift_low)  & 0x0000FFFF;
    const int ret_high = (__shfl(x, src_laneid_high, WARP_SIZE) << shift_high) & 0xFFFF0000;

    return ret_low | ret_high;
}

static __device__ __forceinline__ half2 ggml_hip_movmatrix(const half2 x) {
    half2 ret;
    *((int *) &ret) = ggml_hip_movmatrix(*((const int *) &x));
    return ret;
}

namespace ggml_hip_mma {
    ...
    template <int I_, int J_>
    struct tile<I_, J_, __hip_bfloat162, DATA_LAYOUT_I_MAJOR> {
        static constexpr int         I  = I_;
        static constexpr int         J  = J_;
        static constexpr data_layout dl = DATA_LAYOUT_I_MAJOR;
        ...
        __hip_bfloat162 x[ne] = {{0.0f, 0.0f}};
    ...
    template <int I_, int J_>
    struct tile<I_, J_, __hip_bfloat162, DATA_LAYOUT_I_MAJOR_MIRRORED> {
        static constexpr int         I  = I_;
        static constexpr int         J  = J_;
        static constexpr data_layout dl = DATA_LAYOUT_I_MAJOR_MIRRORED;
        static constexpr int         ne = tile<I_, J_, float, DATA_LAYOUT_I_MAJOR_MIRRORED>::ne;
    ...
    struct tile<I_, J_, half2, DATA_LAYOUT_I_MAJOR_SCRAMBLED> {
        ...
        static constexpr int ne = I * J / ggml_hip_get_physical_warp_size();
        half2 x[ne] = {{0.0f, 0.0f}};
...
#pragma unroll
        for (int l = 0; l < tile_float.ne; ++l) {
            ...
            i ^= 1;
            tmp[i] = __shfl_xor(tile_float.x[l], 16, WARP_SIZE);
...
#pragma unroll
        for (int l0 = 0; l0 < tile_float.ne; l0 += 4) {
            ...
            ret.x[l0/2 + (((threadIdx.x % 4) / 2) ^ 1)] = __shfl_xor(ret.x[l0/2 + (((threadIdx.x % 4) / 2) ^ 1)], 2, WARP_SIZE);
        }
        return ret;
    }
...
#pragma unroll
    ...
    static __device__ __forceinline__ void load_ldmatrix(
        ...
        ggml_hip_memcpy_1<8>(t.x + 0, xs0 + t.get_i(0)*stride + 0);
        ggml_hip_memcpy_1<8>(t.x + 2, xs0 + t.get_i(0)*stride + 2);
#else
        static_assert(dl == DATA_LAYOUT_I_MAJOR, "bad data layout");
        static_assert(sizeof(t.x) == 8, "bad ne");
        ggml_hip_memcpy_1<8>(t.x, xs0 + t.get_i(0)*stride + t.get_j(0));
#endif // RDNA3
#elif defined(AMD_MFMA_AVAILABLE)
        static_assert(sizeof(t.x) == 4, "bad ne");
        ggml_hip_memcpy_1<4>(t.x, xs0 + t.get_i(0)*stride + t.get_j(0));
#else
        GGML_UNUSED_VARS(t, xs0, stride);
        NO_DEVICE_CODE;
    ...
    static __device__ __forceinline__ void load_ldmatrix(
        ...
        ggml_hip_memcpy_1<16>(t.x + 0, xs0 + t.get_i(0)*stride + 0);
        ggml_hip_memcpy_1<16>(t.x + 4, xs0 + t.get_i(0)*stride + 4);
#else
        static_assert(dl == DATA_LAYOUT_I_MAJOR, "bad data layout");
        static_assert(sizeof(t.x) == 16, "bad ne");
        ggml_hip_memcpy_1<16>(t.x, xs0 + t.get_i(0)*stride + t.get_j(0));
#endif // RDNA3
#elif defined(AMD_MFMA_AVAILABLE)
        static_assert(sizeof(t.x) == 8, "bad ne");
        ggml_hip_memcpy_1<8>(t.x, xs0 + t.get_i(0)*stride + t.get_j(0));
    ...
    static __device__ __forceinline__ void load_ldmatrix(
        ...
        ggml_hip_memcpy_1<4*sizeof(half2)>(t.x, xs0 + t.get_i(0)*stride);
    }
...
#pragma unroll
        for (int l0 = 0; l0 < t.ne; l0 += 2) {
            ggml_hip_memcpy_1<2*sizeof(half2)>(t.x + l0, xs0 + t.get_i(l0)*stride + t.get_j(l0));
        }
    }
...
#pragma unroll
    ...
    static __device__ __forceinline__ void mma(
            tile<16, 8, float> & D, const tile<16, 8, __hip_bfloat162> & A, const tile<8, 8, __hip_bfloat162> & B) {
        GGML_UNUSED_VARS(D, A, B);
        NO_DEVICE_CODE;
    }
    ...
    template <data_layout dl_ab, data_layout dl_d>
    static __device__ __forceinline__ void mma(
            tile<16, 16, float, dl_d> & D, const tile<16, 8, __hip_bfloat162, dl_ab> & A, const tile<16, 8, __hip_bfloat162, dl_ab> & B) {
#if defined(AMD_WMMA_AVAILABLE)
#if defined(RDNA4)
