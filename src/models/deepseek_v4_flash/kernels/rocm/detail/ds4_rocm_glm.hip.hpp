...
static int glm_rocm_tensor_has_cache2(
        ...
        uint64_t b,
        uint64_t elem_size) {
    return hip_tensor_has_elems2(t, a, b, elem_size);
}
...
static int glm_rocm_tensor_has_cache3(
        ...
        uint64_t c,
        uint64_t elem_size) {
    return hip_tensor_has_elems3(t, a, b, c, elem_size);
}
...
static int glm_rocm_model_f32_range(
        ...
        uint64_t *bytes_out) {
    uint64_t bytes = 0;
    if (!hip_u64_mul_checked(elems, sizeof(float), &bytes) ||
        !hip_model_range_fits(model_size, offset, bytes)) {
        return 0;
    }
...
static int glm_rocm_check_q8_rows(
    ...
    if (!hip_u64_mul_checked(blocks, 34u, &row_bytes) ||
        !hip_u64_mul_checked(rows, row_bytes, &bytes) ||
        !hip_model_range_fits(model_size, weight_offset, bytes)) {
        return 0;
    }
    const char *ptr = hip_model_range_ptr(model_map, weight_offset, bytes, label);
    if (!ptr) return 0;
...
extern "C" int ds4_gpu_glm_stream_expert_cache_begin_selected_load_tensor(
    ...
    if (!table || n_selected > DS4_ROCM_N_EXPERT_USED ||
        (n_selected != 0u && !hip_tensor_has_i32(selected, n_selected))) {
        return 0;
    }
...
extern "C" int ds4_gpu_glm_kv_lora_rms_norm_tensor(
    ...
    if (!out || !kv_raw || !model_map || n_tokens == 0u ||
        ...
        !hip_tensor_has_elems2(kv_raw, n_tokens, kv_raw_dim, sizeof(float)) ||
        !hip_tensor_has_elems2(out, n_tokens, kv_lora_dim, sizeof(float)) ||
        !glm_rocm_model_f32_range(model_size, weight_offset, kv_lora_dim, &weight_bytes)) {
        return 0;
    }
    const float *w = (const float *)hip_model_range_ptr(model_map,
                                                         weight_offset,
                                                         weight_bytes,
                                                         "glm_kv_lora_norm");
    if (!w) return 0;
    glm_kv_lora_rms_norm_kernel<<<n_tokens, 256>>>((float *)out->ptr,
                                                   ...
                                                   kv_lora_dim,
                                                   eps);
    return hip_ok(hipGetLastError(), "glm kv_lora rms norm launch");
}
...
extern "C" int ds4_gpu_glm_k_b_project_tensor(
    ...
    if (!out || !kv_norm || !model_map || n_tokens == 0u ||
        ...
        !hip_tensor_has_elems2(kv_norm, n_tokens, kv_lora_dim, sizeof(float)) ||
        !hip_tensor_has_elems3(out, n_tokens, n_head, qk_nope, sizeof(float)) ||
    ...
    glm_k_b_project_q8_0_head_kernel<<<grid, 256, (size_t)kv_lora_dim * sizeof(float)>>>(
            ...
            n_head,
            row_bytes);
    return hip_ok(hipGetLastError(), "glm k_b project launch");
}
...
extern "C" int ds4_gpu_glm_store_compact_kv_tensor(
    ...
    glm_store_compact_kv_kernel<<<grid, 256>>>((char *)kv_lora_cache->ptr,
                                               ...
                                               qk_rope,
                                               cache_f16);
    return hip_ok(hipGetLastError(), "glm store compact kv launch");
}
...
extern "C" int ds4_gpu_glm_qkv_norm_store_compact_kv_tensor(
    ...
    if (!q_out || !q || !model_map || !kv_lora_cache || !k_rope_cache || !kv_raw ||
        ...
        !hip_tensor_has_elems2(q, n_tokens, q_n, sizeof(float)) ||
        !hip_tensor_has_elems2(q_out, n_tokens, q_n, sizeof(float)) ||
        !hip_tensor_has_elems2(kv_raw, n_tokens, kv_raw_dim, sizeof(float)) ||
        !glm_rocm_tensor_has_cache2(kv_lora_cache, cache_cap, kv_lora_dim, elem) ||
        !glm_rocm_tensor_has_cache2(k_rope_cache, cache_cap, qk_rope, elem) ||
    ...
    const float *qw = (const float *)hip_model_range_ptr(model_map, q_weight_offset,
                                                          q_weight_bytes,
                                                          "glm_q_norm");
    const float *kvw = (const float *)hip_model_range_ptr(model_map, kv_weight_offset,
                                                           kv_weight_bytes,
                                                           "glm_kv_norm");
    if (!qw || !kvw) return 0;
    ...
    glm_qkv_norm_store_compact_kv_kernel<<<grid, 256>>>((float *)q_out->ptr,
                                                        ...
                                                        cache_f16,
                                                        eps);
    return hip_ok(hipGetLastError(), "glm qkv norm compact kv launch");
}
...
extern "C" int ds4_gpu_glm_store_indexer_k_tensor(
    ...
    if (!indexer_key_cache || !raw_k || !model_map || n_tokens == 0u ||
        ...
        !hip_tensor_has_elems2(raw_k, n_tokens, head_dim, sizeof(float)) ||
        ...
        !hip_model_range_fits(model_size, bias_offset, bytes)) {
        return 0;
    }
    const float *w = (const float *)hip_model_range_ptr(model_map, weight_offset, bytes, "glm_indexer_weight");
    const float *b = (const float *)hip_model_range_ptr(model_map, bias_offset, bytes, "glm_indexer_bias");
    if (!w || !b) return 0;
    glm_store_indexer_k_kernel<<<n_tokens, 256>>>((char *)indexer_key_cache->ptr,
                                                  ...
                                                  beta_slow,
                                                  cache_f16);
    return hip_ok(hipGetLastError(), "glm store indexer k launch");
}
...
extern "C" int ds4_gpu_glm_build_kv_cache_tensor(
    ...
    if (!key_cache || !value_cache || !kv_raw || !k_nope || !value ||
        ...
        !hip_tensor_has_elems2(kv_raw, n_tokens, kv_raw_dim, sizeof(float)) ||
        !hip_tensor_has_elems3(k_nope, n_tokens, n_head, qk_nope, sizeof(float)) ||
        !hip_tensor_has_elems3(value, n_tokens, n_head, value_dim, sizeof(float)) ||
    ...
    glm_build_kv_cache_kernel<<<grid, 256>>>((char *)key_cache->ptr,
                                             ...
                                             beta_slow,
                                             cache_f16);
    return hip_ok(hipGetLastError(), "glm build kv cache launch");
}
...
extern "C" int ds4_gpu_glm_attention_full_tensor(
    ...
    if (!heads || !q || !key_cache || !value_cache || n_tokens == 0u ||
        ...
        !hip_attention_score_buffer_fits(cache_len) ||
        !hip_tensor_has_elems3(q, n_tokens, n_head, qk_dim, sizeof(float)) ||
        !hip_tensor_has_elems3(heads, n_tokens, n_head, value_dim, sizeof(float)) ||
        !glm_rocm_tensor_has_cache3(key_cache, cache_cap, n_head, qk_dim, elem) ||
        !glm_rocm_tensor_has_cache3(value_cache, cache_cap, n_head, value_dim, elem)) {
        return 0;
    }
    ...
    glm_attention_full_kernel<<<grid, 256, shmem>>>((float *)heads->ptr,
                                                    ...
                                                    value_dim,
                                                    cache_f16);
    return hip_ok(hipGetLastError(), "glm attention full launch");
}
...
extern "C" int ds4_gpu_glm_fill_selected_range_tensor(ds4_gpu_tensor *selected, uint32_t n_selected) {
    ...
    if (!hip_tensor_has_i32(selected, n_selected)) return 0;
    glm_fill_selected_range_kernel<<<(n_selected + 255u) / 256u, 256>>>((int32_t *)selected->ptr, n_selected);
    return hip_ok(hipGetLastError(), "glm fill selected range launch");
}
...
extern "C" int ds4_gpu_glm_fill_selected_range_batch_tensor(
    ...
    uint64_t total = 0;
    uint32_t blocks = 0;
    if (!hip_u64_mul_checked(n_tokens, n_selected, &total) ||
    ...
    glm_fill_selected_range_batch_kernel<<<blocks, 256>>>((int32_t *)selected->ptr,
                                                                         ...
                                                                         n_selected,
                                                                         pad_row);
    return hip_ok(hipGetLastError(), "glm fill selected range batch launch");
}
...
extern "C" int ds4_gpu_glm_indexer_rope_tail_tensor(
    ...
    if (!x || rot_dim > head_dim || (rot_dim & 1u) ||
        ...
        !hip_tensor_has_elems3(x, n_tokens, n_head, head_dim, sizeof(float))) {
        return 0;
    }
    uint64_t token_heads = 0;
    uint64_t pairs = 0;
    uint32_t blocks = 0;
    if (!hip_u64_mul_checked(n_tokens, n_head, &token_heads) ||
        !hip_u64_mul_checked(token_heads, rot_dim >> 1u, &pairs)) {
        return 0;
    }
    ...
    glm_indexer_rope_tail_kernel<<<blocks, 256>>>((float *)x->ptr,
                                                                 ...
                                                                 beta_fast,
                                                                 beta_slow);
    return hip_ok(hipGetLastError(), "glm indexer rope tail launch");
}
...
extern "C" int ds4_gpu_glm_indexer_score_one_tensor(
    ...
    if (!scores || !q || !weights || !indexer_key_cache ||
        ...
        !hip_tensor_has_f32(scores, n_rows) ||
        !hip_tensor_has_elems2(q, n_head, head_dim, sizeof(float)) ||
        !hip_tensor_has_f32(weights, n_head) ||
    ...
    glm_indexer_score_one_kernel<<<n_rows, 256>>>((float *)scores->ptr,
                                                  ...
                                                  scale,
                                                  cache_f16);
    return hip_ok(hipGetLastError(), "glm indexer score one launch");
}
...
extern "C" int ds4_gpu_glm_indexer_scores_batch_tensor(
    ...
    if (!scores || !q || !weights || !indexer_key_cache || n_tokens == 0u ||
        ...
        !glm_rocm_check_token_span(pos0, n_tokens, &end_pos) ||
        end_pos > n_rows ||
        !hip_tensor_has_elems2(scores, n_tokens, n_rows, sizeof(float)) ||
        !hip_tensor_has_elems3(q, n_tokens, n_head, head_dim, sizeof(float)) ||
        !hip_tensor_has_elems2(weights, n_tokens, n_head, sizeof(float)) ||
        !glm_rocm_tensor_has_cache2(indexer_key_cache, n_rows, head_dim, elem)) {
        return 0;
    }
    ...
    glm_indexer_scores_batch_kernel<<<grid, 256>>>((float *)scores->ptr,
                                                   ...
                                                   scale,
                                                   cache_f16);
    return hip_ok(hipGetLastError(), "glm indexer scores batch launch");
}
...
extern "C" int ds4_gpu_glm_qk_lowrank_q8_0_tensor(
    ...
    if (!qk_low || !q || !model_map ||
        ...
        !hip_u64_mul_checked(n_head, qk_dim, &x_stride64) ||
        x_stride64 > UINT32_MAX ||
        !hip_tensor_has_elems2(q, n_head, qk_dim, sizeof(float)) ||
        !hip_tensor_has_elems2(qk_low, n_head, kv_lora_dim, sizeof(float)) ||
        !glm_rocm_check_q8_rows(model_map, model_size, weight_offset,
                                (uint64_t)n_head * kv_lora_dim, qk_nope,
    ...
    glm_q8_project_head_kernel<<<dim3(n_head, 1, 1), 256, (size_t)qk_nope * sizeof(float)>>>(
            ...
            qk_dim,
            row_bytes);
    return hip_ok(hipGetLastError(), "glm qk lowrank launch");
}
...
extern "C" int ds4_gpu_glm_qk_lowrank_q8_0_batch_tensor(
    ...
    if (!qk_low || !q || !model_map || n_tokens == 0u ||
        ...
        x_stride64 > UINT32_MAX ||
        !hip_tensor_has_elems3(q, n_tokens, n_head, qk_dim, sizeof(float)) ||
        !hip_tensor_has_elems3(qk_low, n_tokens, n_head, kv_lora_dim, sizeof(float)) ||
        !glm_rocm_check_q8_rows(model_map, model_size, weight_offset,
                                (uint64_t)n_head * kv_lora_dim, qk_nope,
    ...
    glm_q8_project_head_kernel<<<grid, 256, (size_t)qk_nope * sizeof(float)>>>(
            ...
            qk_dim,
            row_bytes);
    return hip_ok(hipGetLastError(), "glm qk lowrank batch launch");
}
...
extern "C" int ds4_gpu_glm_value_project_q8_0_batch_heads_tensor(
    ...
    if (!heads || !lora || !model_map || n_tokens == 0u ||
        ...
        kv_lora_dim > DS4_ROCM_ATTENTION_SCORE_CAP ||
        !hip_u64_mul_checked(n_head, kv_lora_dim, &x_stride64) ||
        x_stride64 > UINT32_MAX ||
        !hip_tensor_has_elems3(lora, n_tokens, n_head, kv_lora_dim, sizeof(float)) ||
        !hip_tensor_has_elems3(heads, n_tokens, n_head, value_dim, sizeof(float)) ||
        !glm_rocm_check_q8_rows(model_map, model_size, weight_offset,
    ...
    if (n_tokens > 1u &&
        (kv_lora_dim & 31u) == 0u &&
        hip_runtime_config()->glm_grouped_value_project) {
        const uint32_t rows_per_block = 32u;
        const uint32_t token_tile = 32u;
        const uint32_t block_tile = 16u;
        hip_launch_grouped_q8_a_sharedx(
                (float *)heads->ptr,
                w,
     ...
     */
    if (n_tokens == 1u &&
        (wave_decode_env == NULL || hip_env_present(wave_decode_env))) {
    ...
    glm_q8_project_head_kernel<<<grid, 256, (size_t)kv_lora_dim * sizeof(float)>>>(
            ...
            kv_lora_dim,
            row_bytes);
    return hip_ok(hipGetLastError(), "glm value project batch heads launch");
}
...
static int glm_causal_gemm_scratch_part(
    ...
    if (!offset || !part_offset ||
        !hip_u64_add_checked(*offset, 255u, &aligned)) {
        return 0;
    }
    aligned &= ~255ull;
    if (!hip_u64_add_checked(aligned, bytes, offset)) return 0;
    *part_offset = aligned;
    return 1;
}
...
static hipEvent_t g_glm_selected_profile_begin;
static hipEvent_t g_glm_selected_profile_end;
...
static void glm_selected_gemm_profile_init(
    ...
    if (!g_glm_selected_profile_events_ready) {
        if (hipEventCreate(&g_glm_selected_profile_begin) != hipSuccess) {
            return;
        }
        if (hipEventCreate(&g_glm_selected_profile_end) != hipSuccess) {
            (void)hipEventDestroy(g_glm_selected_profile_begin);
            return;
        }
...
static void glm_selected_gemm_profile_begin(
    ...
    if (hipEventRecord(g_glm_selected_profile_begin, 0) != hipSuccess) {
        profile->enabled = 0;
    }
}
...
static void glm_selected_gemm_profile_end(
    ...
    float elapsed = 0.0f;
    if (hipEventRecord(g_glm_selected_profile_end, 0) != hipSuccess ||
        hipEventSynchronize(g_glm_selected_profile_end) != hipSuccess ||
        hipEventElapsedTime(&elapsed,
                             g_glm_selected_profile_begin,
                             g_glm_selected_profile_end) != hipSuccess) {
        profile->enabled = 0;
        return;
    }
...
static int glm_attention_indexed_lora_causal_gemm(
        ...
        float beta_fast,
        float beta_slow) {
    if (!g_hipblas_ready || !lora_out || !q || !qk_low ||
    ...
    if (!hip_u64_mul_checked(n_selected, kv_lora_dim, &kv_count) ||
        !hip_u64_mul_checked(n_selected, qk_rope, &rope_count) ||
        !hip_u64_mul_checked(n_tokens, kv_lora_dim, &low_count) ||
        !hip_u64_mul_checked(n_tokens, qk_rope, &qrope_count) ||
        !hip_u64_mul_checked(n_tokens, n_selected, &score_count) ||
        !hip_u64_mul_checked(n_tokens, kv_lora_dim, &head_count)) {
        return 0;
    }
    ...
    if (!hip_u64_mul_checked(kv_count, sizeof(__half), &kv_bytes) ||
        !hip_u64_mul_checked(rope_count, sizeof(__half), &rope_bytes) ||
        !hip_u64_mul_checked(low_count, sizeof(__half), &low_bytes) ||
        !hip_u64_mul_checked(qrope_count, sizeof(__half), &qrope_bytes) ||
        !hip_u64_mul_checked(score_count, sizeof(float), &score_bytes) ||
        !hip_u64_mul_checked(score_count, sizeof(__half), &prob_bytes) ||
        !hip_u64_mul_checked(head_count, sizeof(float), &head_bytes)) {
        return 0;
    }
    ...
    char *scratch =
        (char *)hip_tmp_alloc(scratch_bytes, "glm causal attention gemm");
    if (!scratch) return 0;
    ...
    glm_causal_gemm_cache_to_f16_kernel<<<
        (kv_count + 255u) / 256u, 256>>>(
            ...
            kv_count,
            cache_f16);
    if (!hip_ok(hipGetLastError(), "glm causal attention kv f16 launch")) {
        return 0;
    }
    ...
    glm_causal_gemm_rope_to_f16_kernel<<<
        (rope_pairs + 255u) / 256u, 256>>>(
            ...
            beta_fast,
            beta_slow);
    if (!hip_ok(hipGetLastError(), "glm causal attention rope f16 launch")) {
        return 0;
    }
    ...
    for (uint32_t head = 0; head < n_head; head++) {
        ...
        if (!hipblas_ok(st, "glm causal attention lora score gemm")) {
            return 0;
        }
        st = hipblasGemmEx(g_hipblas,
                          ...
                          HIPBLAS_COMPUTE_32F,
                          HIPBLAS_GEMM_DEFAULT);
        if (!hipblas_ok(st, "glm causal attention rope score gemm")) {
            return 0;
        }
        ...
        st = hipblasGemmEx(g_hipblas,
                          ...
                          HIPBLAS_COMPUTE_32F,
                          HIPBLAS_GEMM_DEFAULT);
        if (!hipblas_ok(st, "glm causal attention value gemm")) return 0;
...
static int glm_attention_indexed_lora_selected_gemm(
    ...
    uint64_t low_count = 0, qrope_count = 0;
    uint64_t score_count = 0, head_count = 0;
    if (!hip_u64_mul_checked(n_tokens, n_selected, &selected_rows) ||
        !hip_u64_mul_checked(selected_rows, kv_lora_dim, &kv_count) ||
        !hip_u64_mul_checked(selected_rows, qk_rope, &rope_count) ||
        !hip_u64_mul_checked(n_tokens, kv_lora_dim, &low_count) ||
        !hip_u64_mul_checked(n_tokens, qk_rope, &qrope_count) ||
        !hip_u64_mul_checked(n_tokens, n_selected, &score_count) ||
        !hip_u64_mul_checked(n_tokens, kv_lora_dim, &head_count)) {
    ...
    if (!hip_u64_mul_checked(low_count, head_tile, &low_count) ||
        !hip_u64_mul_checked(qrope_count, head_tile, &qrope_count) ||
        !hip_u64_mul_checked(score_count, head_tile, &score_count) ||
        !hip_u64_mul_checked(head_count, head_tile, &head_count)) {
        return 0;
    }
    ...
    char *scratch =
        (char *)hip_tmp_alloc(scratch_bytes, "glm selected attention gemm");
    if (!scratch) return 0;
    __half *kv_h = (__half *)(scratch + kv_offset);
    __half *rope_h = (__half *)(scratch + rope_offset);
    __half *low_h = (__half *)(scratch + low_offset);
    ...
    for (uint32_t head0 = 0; head0 < n_head; head0 += head_tile) {
        ...
        hipblasStatus_t st = hipblasGemmStridedBatchedEx(
            ...
            HIPBLAS_COMPUTE_32F,
            HIPBLAS_GEMM_DEFAULT);
        if (!hipblas_ok(st, "glm selected attention lora score gemm")) {
            return 0;
        }
        glm_selected_gemm_profile_end(
        ...
        st = hipblasGemmStridedBatchedEx(
            ...
            HIPBLAS_COMPUTE_32F,
            HIPBLAS_GEMM_DEFAULT);
        if (!hipblas_ok(st, "glm selected attention rope score gemm")) {
            return 0;
        }
        glm_selected_gemm_profile_end(
        ...
        st = hipblasGemmStridedBatchedEx(
            ...
            HIPBLAS_COMPUTE_32F,
            HIPBLAS_GEMM_DEFAULT);
        if (!hipblas_ok(st, "glm selected attention value gemm")) {
            return 0;
        }
        glm_selected_gemm_profile_end(
...
static int glm_attention_indexed_lora_launch(
    ...
    if (!lora_out || !q || !qk_low || !kv_lora_cache || !k_rope_cache ||
        ...
        !hip_attention_score_buffer_fits(n_selected) ||
        ...
        !hip_tensor_has_elems3(lora_out, n_tokens, n_head, kv_lora_dim, sizeof(float)) ||
        !hip_tensor_has_elems3(q, n_tokens, n_head, qk_dim, sizeof(float)) ||
        !hip_tensor_has_elems3(qk_low, n_tokens, n_head, kv_lora_dim, sizeof(float)) ||
        (has_selected && !hip_tensor_has_elems2(selected, n_tokens, n_selected, sizeof(int32_t))) ||
        !glm_rocm_tensor_has_cache2(kv_lora_cache, cache_cap, kv_lora_dim, elem) ||
        !glm_rocm_tensor_has_cache2(k_rope_cache, cache_cap, qk_rope, elem)) {
        return 0;
    }
    ...
    if (!causal_range && has_selected &&
        (selected_attn_gemm_env == NULL ||
         hip_env_present(selected_attn_gemm_env)) &&
    ...
    if (causal_range && !has_selected &&
        (causal_attn_gemm_env == NULL ||
         hip_env_present(causal_attn_gemm_env)) &&
    ...
    glm_attention_indexed_lora_kernel<<<grid, 256, shmem>>>((float *)lora_out->ptr,
                                                            ...
                                                            beta_fast,
                                                            beta_slow);
    return hip_ok(hipGetLastError(), "glm indexed lora launch");
}
...
extern "C" int ds4_gpu_glm_attention_indexed_batch_tensor(
    ...
    if (!glm_rocm_u32_add_checked(qk_nope, qk_rope, &qk_dim) ||
        !hip_u64_mul3_checked(n_tokens, n_head, kv_lora_dim, &tmp_elems) ||
        !hip_u64_mul3_checked(n_tokens, n_head, qk_dim, &q_elems) ||
        !hip_u64_mul3_checked(n_tokens, n_head, kv_lora_dim, &low_elems) ||
        !hip_u64_mul3_checked(n_tokens, n_head, value_dim, &heads_elems) ||
        !hip_u64_mul_checked(tmp_elems, sizeof(float), &lora_tmp.bytes) ||
        !hip_u64_mul_checked(q_elems, sizeof(float), &q_bytes) ||
        !hip_u64_mul_checked(low_elems, sizeof(float), &low_bytes) ||
        !hip_u64_mul_checked(heads_elems, sizeof(float), &heads_bytes)) {
        return 0;
    }
    if (!heads || !q || !qk_low || !kv_lora_cache || !k_rope_cache ||
        ...
        !isfinite(ext_factor) || !isfinite(attn_factor) ||
        !isfinite(beta_fast) || !isfinite(beta_slow) ||
        !hip_tensor_has_bytes(q, q_bytes) ||
        !hip_tensor_has_bytes(qk_low, low_bytes) ||
        !hip_tensor_has_elems2(selected, n_tokens, n_selected, sizeof(int32_t)) ||
        !hip_tensor_has_bytes(heads, heads_bytes) ||
        !glm_rocm_tensor_has_cache2(kv_lora_cache, cache_cap, kv_lora_dim, elem) ||
        !glm_rocm_tensor_has_cache2(k_rope_cache, cache_cap, qk_rope, elem)) {
        return 0;
    }
    lora_tmp.ptr = hip_tmp_alloc(lora_tmp.bytes, "glm indexed batch lora");
    lora_tmp.owner = 0;
...
extern "C" int ds4_gpu_glm_attention_indexed_decode_tensor(
    ...
    const uint64_t elem = cache_f16 ? sizeof(__half) : sizeof(float);
    if (!glm_rocm_u32_add_checked(qk_nope, qk_rope, &qk_dim) ||
        !hip_u64_mul3_checked(n_head, qk_dim, sizeof(float), &q_bytes) ||
        !hip_u64_mul3_checked(n_head, kv_lora_dim, sizeof(float), &low_bytes) ||
        !hip_u64_mul3_checked(n_head, value_dim, sizeof(float), &heads_bytes)) {
        return 0;
    }
    if (!heads ||
        !q ||
        ...
        !hip_tensor_has_bytes(q, q_bytes) ||
        !hip_tensor_has_elems2(qk_low, n_head, kv_lora_dim, sizeof(float)) ||
        !hip_tensor_has_i32(selected, n_selected) ||
        !hip_tensor_has_bytes(heads, heads_bytes) ||
        !glm_rocm_tensor_has_cache2(kv_lora_cache, cache_cap, kv_lora_dim, elem) ||
        !glm_rocm_tensor_has_cache2(k_rope_cache, cache_cap, qk_rope, elem)) {
        return 0;
    ...
    lora_tmp.ptr = hip_tmp_alloc(lora_tmp.bytes, "glm indexed decode lora");
    lora_tmp.owner = 0;
    if (!lora_tmp.ptr) return 0;
...
extern "C" int ds4_gpu_glm_attention_indexed_decode_split_group8_tensor(
    ...
    if (!glm_rocm_u32_add_checked(qk_nope, qk_rope, &qk_dim) ||
        ...
        !hip_tensor_has_elems2(qk_low, n_head, kv_lora_dim, sizeof(float)) ||
        !hip_tensor_has_i32(selected, n_selected) ||
        !hip_tensor_has_elems2(heads, n_head, value_dim, sizeof(float)) ||
        !hip_tensor_has_elems3(partial_lora, n_blocks, n_head, kv_lora_dim, sizeof(float)) ||
        !hip_tensor_has_elems3(partial_ms, n_blocks, n_head, 2u, sizeof(float)) ||
    ...
    if (!hip_ok(hipGetLastError(), "glm indexed split partial launch")) return 0;
    ...
    glm_attention_indexed_decode_split_reduce_kernel<<<dim3(n_head, 1, 1),
            ...
            row_bytes,
            n_blocks);
    return hip_ok(hipGetLastError(), "glm indexed split reduce launch");
}
...
static int glm_router_select_launch(
    ...
    if (!selected || !weights || !probs || !logits || !model_map || n_tokens == 0 ||
        ...
        !(active_scale > 0.0f) ||
        !hip_tensor_has_elems2(logits, n_tokens, active_n_expert, sizeof(float)) ||
        !hip_tensor_has_elems2(probs, n_tokens, active_n_expert, sizeof(float)) ||
        !hip_tensor_has_elems2(selected, n_tokens, active_n_expert_used, sizeof(int32_t)) ||
        !hip_tensor_has_elems2(weights, n_tokens, active_n_expert_used, sizeof(float)) ||
        !hip_model_range_fits(model_size, bias_offset, (uint64_t)active_n_expert * sizeof(float))) {
        return 0;
    }

    const float *bias = (const float *)hip_model_range_ptr(
            model_map,
            bias_offset,
    ...
    return hip_ok(hipGetLastError(), "glm_router_select launch");
}
...
static int glm_rocm_routed_moe_wrap(
    ...
    if (!hip_u64_mul3_checked(n_tokens, n_expert, expert_mid_dim, &pair_elems) ||
        !hip_u64_mul_checked(pair_elems, sizeof(float), &gate_bytes) ||
        !hip_u64_mul3_checked(n_tokens, n_expert, out_dim, &pair_elems) ||
        !hip_u64_mul_checked(pair_elems, sizeof(float), &down_bytes) ||
    ...
    char *tmp = (char *)hip_tmp_alloc(tmp_bytes, "glm moe scratch");
    if (!tmp) return 0;
