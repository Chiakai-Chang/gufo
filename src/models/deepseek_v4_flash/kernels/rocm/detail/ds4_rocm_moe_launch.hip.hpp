...
static hipEvent_t g_moe_decode_profile_events[DS4_ROCM_MOE_DECODE_PROFILE_EVENT_COUNT];
...
static int routed_moe_decode_profile_ensure_events(void) {
    ...
    for (uint32_t i = 0; i < DS4_ROCM_MOE_DECODE_PROFILE_EVENT_COUNT; i++) {
        hipError_t err = hipEventCreate(&g_moe_decode_profile_events[i]);
        if (err != hipSuccess) {
            fprintf(stderr,
                    DS4_GPU_LOG_PREFIX "Q2 decode MoE profile event create failed: %s\n",
                    hipGetErrorString(err));
            for (uint32_t j = 0; j < i; j++) {
                (void)hipEventDestroy(g_moe_decode_profile_events[j]);
...
static int routed_moe_decode_profile_record_event(uint32_t ev, const char *what) {
    ...
    return hip_ok(hipEventRecord(g_moe_decode_profile_events[ev], 0), what);
}
...
static int routed_moe_decode_profile_add_event_ms(uint32_t start_ev,
    ...
    if (!hip_ok(hipEventSynchronize(g_moe_decode_profile_events[end_ev]),
                 what)) {
        return 0;
    }
    float ms = 0.0f;
    if (!hip_ok(hipEventElapsedTime(&ms,
...
static int routed_moe_q2_float_down_launch(
    ...
    if (!hip_ok(hipMemcpy(h_counts, counts, n_total_expert * sizeof(uint32_t), hipMemcpyDeviceToHost),
...
static int routed_moe_full_table_is_cached(
        ...
        uint64_t gate_bytes,
        uint64_t down_bytes) {
    return hip_model_range_is_cached(model_map, gate_offset, gate_bytes) &&
...
static int routed_moe_launch(
    ...
    if (!hip_u64_mul_checked(n_tokens, n_expert, &pair_count64) ||
    ...
    const int stream_full_layer =
        ...
        hip_stream_layer_expert_cache_apply(model_map,
                                             layer_index,
                                             n_total_expert,
                                             gate_offset,
                                             up_offset,
    ...
    const int batch_stream_split_selected =
        ...
        hip_stream_batch_selected_apply_split(model_map,
                                               layer_index,
                                               n_total_expert,
                                               n_expert,
                                               n_tokens,
    ...
    const int batch_stream_selected =
        ...
        (iq2_gate_path || q2k_path) &&
        n_expert <= DS4_ROCM_N_EXPERT_USED &&
        hip_stream_batch_selected_prepare(model_map,
                                           model_size,
                                           layer_index,
                                           selected,
                                           n_tokens,
    ...
    const int split_selected =
        ...
        hip_stream_selected_apply_split(model_map,
                                         layer_index,
                                         n_total_expert,
                                         n_expert,
                                         gate_expert_bytes,
    ...
    const int compact_selected =
        ...
        (!stream_full_layer &&
        n_tokens == 1u &&
        hip_stream_selected_apply(model_map,
                                   layer_index,
                                   n_total_expert,
                                   n_expert,
                                   gate_expert_bytes,
    ...
    if (!compact_selected && !batch_stream_selected && !batch_stream_split_selected) {
        ...
        if (!stream_full_layer) {
            gate_w = hip_model_range_ptr(model_map, gate_offset, gate_bytes, "moe_gate");
            up_w = hip_model_range_ptr(model_map, up_offset, gate_bytes, "moe_up");
            down_w = hip_model_range_ptr(model_map, down_offset, down_bytes, "moe_down");
        }
    }
    if (batch_stream_selected || batch_stream_split_selected) {
        ...
        if (!hip_stream_batch_selected_wait_upload_ready()) return 0;
    } else if (!gate_w || !up_w || !down_w) {
        return 0;
    }
    if (compact_selected && !hip_stream_selected_wait_upload_ready()) return 0;
    ...
    if (!hip_u64_mul_checked(n_tokens, xq_blocks, &xq_count) ||
        !hip_u64_mul_checked(pair_count64, midq_blocks, &midq_count) ||
        !hip_u64_mul_checked(xq_count, sizeof(hip_block_q8_K), &xq_bytes) ||
        !hip_u64_mul_checked(midq_count, sizeof(hip_block_q8_K), &midq_bytes)) {
        return 0;
    }
    if (!q2k_path && down->bytes >= xq_bytes && gate->bytes >= midq_bytes) {
        ...
        if (ok && (batch_stream_selected || batch_stream_split_selected)) {
            ...
            if (batch_stream_split_selected) {
                ...
                if (!ok) {
                    (void)hip_stream_batch_selected_finish_pending_missing();
                } else {
                    ok = hip_stream_batch_selected_finish_pending_missing();
                }
            ...
            if (ok) {
                ...
                q8_K_quantize_kernel<<<midq_grid, 256>>>(
                        ...
                        expert_mid_dim,
                        pair_count);
                ok = hip_ok(hipGetLastError(), "routed_moe streaming batch mid quantize launch");
            }
            if (ok) {
            ...
            if (ok) ok = hip_stream_batch_selected_mark_inflight();
            return ok;
        }
        ...
        if (use_iq2_gate_wmma) {
            uint32_t h_counts[DS4_ROCM_MAX_N_EXPERT] = {0};
            if (!hip_ok(hipMemcpy(h_counts, sorted_counts, n_total_expert * sizeof(uint32_t), hipMemcpyDeviceToHost),
        ...
        if (ok && split_selected) {
            ...
            if (split_supported) {
                ...
                ok = hip_ok(hipGetLastError(), "routed_moe split resident gate/up launch");
                if (!ok) {
                    (void)hip_stream_selected_finish_pending_missing(0);
                } else {
                    ok = hip_stream_selected_finish_pending_missing(0);
                }
                ...
                if (ok) ok = hip_ok(hipGetLastError(), "routed_moe split missing gate/up launch");
                split_gateup_done = ok;
            } else {
                ok = hip_stream_selected_finish_pending_missing(
        ...
        if (ok && iq2_iq2_path) {
            ...
            ok = hip_ok(hipGetLastError(), "routed_moe iq2 down launch");
        ...
        if (ok && !direct_iq2_down_done && split_gateup_done) {
            moe_down_sum6_qwarp32_ptrs_kernel<<<(out_dim + 31u) / 32u, 256>>>(
                    ...
                    out_dim,
                    n_expert);
            ok = hip_ok(hipGetLastError(), "routed_moe split ptr down launch");
        ...
        if (ok && compact_selected) ok = hip_stream_selected_mark_inflight();
        return ok;
    }
    ...
    if (q2k_path && n_tokens >= 32u && !cfg->graph_dump) {
        ...
        if (!hip_u64_mul_checked(pair_count64, sizeof(uint32_t), &sorted_bytes) ||
            (moe_wmma_hot && (
                !hip_u64_mul3_checked(pair_count64, expert_mid_dim, sizeof(__half), &f16_mid_bytes) ||
                !hip_u64_mul3_checked(pair_count64, out_dim, sizeof(__half), &f16_down_bytes) ||
                !hip_u64_mul3_checked(n_tokens, expert_in_dim, sizeof(__half), &wmma_x_bytes)))) {
            return 0;
        }
        ...
        if (moe_wmma_hot) {
            if (!hip_ok(hipMemcpy(h_counts, counts, bucket_count * sizeof(uint32_t), hipMemcpyDeviceToHost),
    ...
    if (q2k_path) {
        ...
        if (split_selected) {
            ...
            const double finish_missing_t0 =
                decode_profile ? hip_wall_sec() : 0.0;
            ok_gateup = hip_stream_selected_finish_pending_missing(0);
            if (decode_profile) {
                g_moe_decode_profile_stats.finish_missing_ms +=
                    (hip_wall_sec() - finish_missing_t0) * 1000.0;
            }
        ...
        if (ok_decode_moe && compact_selected) {
            ok_decode_moe = hip_stream_selected_mark_inflight();
        }
