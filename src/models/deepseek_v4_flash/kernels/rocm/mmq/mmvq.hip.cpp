...
#include "unary.hip.hpp"
#include "vecdotq.hip.hpp"

#include <cstdint>

typedef float (*ds4_vec_dot_q_hip_t)(const void * __restrict__ vbq, const block_q8_1 * __restrict__ bq8_1, const int & kbx, const int & iqs);

static constexpr __device__ ds4_vec_dot_q_hip_t ds4_get_vec_dot_q_hip(ggml_type type) {
    switch (type) {
...
static __host__ mmvq_parameter_table_id get_device_table_id(int cc) {
    if (DS4_GGML_HIP_CC_IS_RDNA4(cc)) {
        return MMVQ_PARAMETERS_RDNA4;
    }
    if (DS4_GGML_HIP_CC_IS_RDNA3_0(cc)) {
        return MMVQ_PARAMETERS_RDNA3_0;
    }
    if (DS4_GGML_HIP_CC_IS_RDNA2(cc) || DS4_GGML_HIP_CC_IS_RDNA3_5(cc)) {
        return MMVQ_PARAMETERS_RDNA2;
    }
    if (DS4_GGML_HIP_CC_IS_GCN(cc) || DS4_GGML_HIP_CC_IS_CDNA(cc)) {
        return MMVQ_PARAMETERS_GCN;
    }
    return MMVQ_PARAMETERS_GENERIC;
}
...
int get_mmvq_mmid_max_batch(ggml_type type, int cc) {
    ...
    if (DS4_GGML_HIP_CC_IS_NVIDIA(cc)) {
        if (cc == DS4_GGML_HIP_CC_VOLTA || cc >= DS4_GGML_HIP_CC_ADA_LOVELACE) {
            return MMVQ_MAX_BATCH_SIZE;
        }
        if (cc >= DS4_GGML_HIP_CC_TURING) {
    ...
    if (DS4_GGML_HIP_CC_IS_AMD(cc)) {
        ...
        if (DS4_GGML_HIP_CC_IS_RDNA1(cc) || DS4_GGML_HIP_CC_IS_RDNA2(cc)) {
            return get_mmvq_mmid_max_batch_rdna1_rdna2(type);
        }
        if (DS4_GGML_HIP_CC_IS_CDNA(cc)) {
            return get_mmvq_mmid_max_batch_cdna(type);
        }
        if (DS4_GGML_HIP_CC_IS_GCN(cc)) {
...
static constexpr __device__ int get_mmvq_mmid_max_batch_for_device() {
    ...
    return get_mmvq_mmid_max_batch_gcn(type);
#else
    // Host compilation has no device architecture macros.
    return MMVQ_MAX_BATCH_SIZE;
#endif
}
...
__launch_bounds__(calc_nwarps(type, ncols_dst, get_device_table_id())*ds4_ggml_hip_get_physical_warp_size(), 1)
static __global__ void mul_mat_vec_q(
        const void * __restrict__ vx, const void * __restrict__ vy, const int32_t * __restrict__ ids, const ds4_ggml_hip_mm_fusion_args_device fusion, float * __restrict__ dst,
    ...
    constexpr int rows_per_hip_block = calc_rows_per_block(ncols_dst, table_id, small_k, nwarps);
    constexpr int warp_size = ds4_ggml_hip_get_physical_warp_size();

    constexpr ds4_vec_dot_q_hip_t ds4_vec_dot_q_hip = ds4_get_vec_dot_q_hip(type);

    const     int tid = warp_size*threadIdx.y + threadIdx.x;
    const     int row0 = rows_per_hip_block*blockIdx.x;
    ...
    if constexpr (has_fusion) {
        ...
        if (use_bias) {
            ...
            if (threadIdx.x < rows_per_hip_block && threadIdx.y == 0 &&
                (rows_per_hip_block == 1 || uint32_t(row0 + threadIdx.x) < stride_col_dst)) {
#pragma unroll
...
#pragma unroll
    ...
    float tmp[ncols_dst][rows_per_hip_block] = {{0.0f}};
    float tmp_gate[ncols_dst][rows_per_hip_block] = {{0.0f}};
...
#pragma unroll
        for (int j = 0; j < ncols_dst; ++j) {
#pragma unroll
            for (int i = 0; i < rows_per_hip_block; ++i) {
                tmp[j][i] += ds4_vec_dot_q_hip(
                    vx, &y[j*stride_col_y + kby], kbx_offset + i*stride_row_x + kbx, kqs);
                if constexpr (has_fusion) {
                    if (use_gate) {
                        tmp_gate[j][i] += ds4_vec_dot_q_hip(
    ...
    __shared__ float tmp_shared[nwarps-1 > 0 ? nwarps-1 : 1][ncols_dst][rows_per_hip_block][warp_size];
    __shared__ float tmp_shared_gate[(has_fusion && (nwarps-1 > 0)) ? nwarps-1 : 1][ncols_dst][rows_per_hip_block][warp_size];
    if constexpr (!has_fusion) {
...
#pragma unroll
        ...
        if (threadIdx.x < rows_per_hip_block && (rows_per_hip_block == 1 || uint32_t(row0 + threadIdx.x) < stride_col_dst)) {
                ...
                if (use_gate) {
                    ...
                    switch (active_glu) {
                        case GGML_GLU_OP_SWIGLU:
                            result *= ds4_ggml_hip_op_silu_single(gate_value);
                            break;
                        case GGML_GLU_OP_GEGLU:
                            result *= ds4_ggml_hip_op_gelu_single(gate_value);
                            break;
                        case GGML_GLU_OP_SWIGLU_OAI: {
                            result = ds4_ggml_hip_op_swiglu_oai_single(gate_value, result);
                            break;
                        }
                        default:
...
__launch_bounds__(get_mmvq_mmid_max_batch_for_device<type>()*ds4_ggml_hip_get_physical_warp_size(), 1)
...
#pragma unroll
        for (int i = 0; i < c_rows_per_block; ++i) {
            tmp[i] += ds4_vec_dot_q_hip(vx, &y[kby], kbx_offset + i*stride_row_x + kbx, kqs);
        }
    }
...
static void mul_mat_vec_q_switch_fusion(
        const void * vx, const void * vy, const int32_t * ids, const ds4_ggml_hip_mm_fusion_args_device fusion, float * dst,
        const uint32_t ncols_x, const uint3 nchannels_y, const uint32_t stride_row_x, const uint32_t stride_col_y,
        const uint32_t stride_col_dst, const uint3 channel_ratio, const uint32_t stride_channel_x,
        const uint32_t stride_channel_y, const uint32_t stride_channel_dst, const uint3 sample_ratio,
        const uint32_t stride_sample_x, const uint32_t stride_sample_y, const uint32_t stride_sample_dst,
        const dim3 & block_nums, const dim3 & block_dims, const int nbytes_shared,
        const uint32_t ids_stride, hipStream_t stream) {
...
static void mul_mat_vec_q_moe_launch(
        ...
        const uint32_t stride_channel_x, const uint32_t stride_channel_y, const uint32_t stride_channel_dst,
        const uint32_t ncols_dst, const uint32_t ids_stride,
        const int warp_size, const int nchannels_dst, hipStream_t stream) {
...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    const int device = ds4_ggml_hip_get_device();
    const int                     cc        = ds4_ggml_hip_info().devices[device].cc;
    const int warp_size = ds4_ggml_hip_info().devices[device].warp_size;
    ...
    const auto should_use_small_k = [&](int c_ncols_dst) {
        ...
        const bool is_nvidia_turing_plus  = DS4_GGML_HIP_CC_IS_NVIDIA(cc) && cc >= DS4_GGML_HIP_CC_TURING;
        const bool is_nvidia_pascal_older = DS4_GGML_HIP_CC_IS_NVIDIA(cc) && cc < DS4_GGML_HIP_CC_VOLTA;
...
// are needed for parity comparisons against llama.cpp.
