...
/* Model-file ranges assigned to ROCM devices by the multi-GPU placement
...
int ds4_gpu_matmul_q8_0_kslice_tensor(
        ...
        const ds4_gpu_tensor *x,
        uint64_t                x_elem_off);
/* ROCM multi-row variant. Each input row contains only the owned contiguous
...
/* ROCM batch path: fold an input RMS normalization into the FP16 activation
 ...
 * The row table is copied into ROCM launch parameters, so no device-side
...
/* Decode-island ROCM graph capture (ROCM backend; Metal/ROCm/CPU stub it
 ...
 * the captured kernels bake in.  ds4_rocm.hip.cpp mirrors this struct
