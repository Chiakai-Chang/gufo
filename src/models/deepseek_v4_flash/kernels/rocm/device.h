...
typedef struct ds4_gpu_config {
    int    device_indices[DS4_MAX_GPUS];   /* ROCM device IDs to use */
     ...
     * hipMemGetInfo) is the caller's job; see CLI flag wiring for the
...
typedef struct {
    int    device_id;
    void  *stream;             /* hipStream_t under ROCM */
    void  *hipblas;             /* hipblasHandle_t under ROCM */
    int    hipblas_ready;
    void  *scratch;
    size_t scratch_bytes;
    size_t budget_bytes;
    size_t used_bytes;
    void  *boundary_event;     /* hipEvent_t under ROCM */
} ds4_gpu_ctx;
...
/* Cross-device tensor copy. Same-device → hipMemcpyAsync; peer-capable
 * cross-device → hipMemcpyPeerAsync with event sync; non-peer → pinned
...
/* Cross-device float add used by ROCM tensor-parallel reductions.
...
/* Set the current ROCM device by LOGICAL tier index (0..g_n_gpus-1).
 *
 ...
 * hipSetDevice. Returns 0 on success, nonzero on error or if the
 ...
 * DS4_ROCM_COPY_MODEL environment-variable branch that
 ...
 * caller-supplied expected_device (a PHYSICAL ROCM device id). Otherwise
 * returns 0 — no host-pointer fallback, no different-device match. The
 * caller is expected to have hipSetDevice'd to expected_device before
