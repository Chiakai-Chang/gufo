...
extern "C" int ds4_gpu_attention_noncausal_raw_batch_heads_tensor(
    ...
    if (verify_left > 0) {
        ...
        (void)hipMemcpy(hq.data(), q->ptr, qn * sizeof(float),
                         hipMemcpyDeviceToHost);
        (void)hipMemcpy(hkv.data(), raw_kv->ptr, kn * sizeof(float),
                         hipMemcpyDeviceToHost);
        (void)hipMemcpy(hout.data(), heads->ptr, qn * sizeof(float),
                         hipMemcpyDeviceToHost);
        (void)hipMemcpy(hsink.data(), sinks, (uint64_t)n_head * sizeof(float),
                         hipMemcpyDeviceToHost);
        double max_abs = 0.0;
        double max_rel = 0.0;
...
extern "C" int ds4_gpu_attention_indexed_mixed_batch_heads_tensor(
    ...
    if (n_tokens > 1u && top_k == 512u) {
        ...
        int32_t *sorted = (int32_t *)hip_tmp_alloc(sort_bytes, "indexed attention topk sort");
        if (!sorted) return 0;
...
extern "C" int ds4_gpu_attention_output_q8_batch_f16_tensor(
        ...
        uint32_t                n_tokens) {
    (void)low;
    if (!out_h || !heads || !model_map || !g_hipblas_ready || g_quality_mode ||
    ...
    const __half *out_a_f16 = hip_q8_f16_ptr(model_map, out_a_offset, out_a_bytes,
    ...
    void *tmp = hip_tmp_alloc(tmp_bytes, "attention output f16 out hipblas");
    if (!tmp) return 0;
    ...
    attention_pack_group_heads_f16_kernel<<<(heads_h_count + 255u) / 256u, 256>>>(
            ...
            n_groups,
            group_dim);
    if (!hip_ok(hipGetLastError(), "attention_output_f16 heads pack launch")) return 0;
...
extern "C" int ds4_gpu_attention_output_q8_batch_tensor(
    ...
    if (out_a_f16) {
        if (hip_runtime_config()->attention_output_hipblas_all &&
            !g_quality_mode && !hip_runtime_config()->graph_dump) {
    ...
    if (attn_output_hipblas && !g_quality_mode) {
