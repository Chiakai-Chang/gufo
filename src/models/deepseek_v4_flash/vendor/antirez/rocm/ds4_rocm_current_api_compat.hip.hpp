extern "C" int ds4_gpu_tensor_read_after_selected_event(
    ...
    if (!g_selected_readback_stream) {
        hipError_t err =
            hipStreamCreateWithFlags(&g_selected_readback_stream,
    ...
    hipError_t err = hipStreamWaitEvent(g_selected_readback_stream,
                                         g_selected_readback_event,
                                         0);
#else
    hipError_t err = hipStreamWaitEvent(g_selected_readback_stream,
    ...
    err = hipStreamSynchronize(g_selected_readback_stream);
    if (err != hipSuccess) {
        fprintf(stderr,
...
extern "C" int ds4_gpu_tensor_copy_f32_to_f16(
    ...
    f32_to_f16_kernel<<<(count + 255u) / 256u, 256>>>(
            ...
            count);
    return hip_ok(hipGetLastError(), "tensor copy f32 to f16 launch");
}
...
extern "C" int ds4_gpu_stream_expert_cache_seed_selected(
        ...
        uint32_t                           n_selected) {
    if (!table) return 0;
    if (!hip_stream_selected_load(table->model_map,
                                   table->model_size,
                                   table->layer,
                                   selected_ids,
    ...
    return hip_stream_selected_finish_pending_missing(0);
}
...
extern "C" int ds4_gpu_stream_expert_cache_begin_selected_load(
        ...
        uint32_t                           n_selected) {
    if (!table) return 0;
    return hip_stream_selected_load(table->model_map,
                                     table->model_size,
                                     table->layer,
                                     selected_ids,
                                     table->n_total_expert,
...
extern "C" int ds4_gpu_stream_expert_cache_prepare_selected_batch(
    ...
    uint32_t unique = 0;
    return hip_stream_batch_selected_prepare_from_host(table->model_map,
                                                        table->model_size,
                                                        table->layer,
                                                        selected_ids,
                                                        n_tokens,
...
extern "C" int ds4_gpu_stream_expert_cache_load_layer(
        const ds4_gpu_stream_expert_table *table) {
    if (!table) return 0;
    return hip_stream_layer_expert_cache_load(table->model_map,
                                               table->model_size,
                                               table->layer,
                                               table->n_total_expert,
                                               table->gate_offset,
...
extern "C" int ds4_gpu_stream_expert_cache_seed_from_layer_selected(
        ...
        uint32_t                          n_selected) {
    if (!table) return 0;
    return hip_stream_layer_expert_cache_seed_selected(table->model_map,
                                                        table->model_size,
                                                        table->layer,
                                                        selected,
                                                        n_tokens,
...
extern "C" int ds4_gpu_stream_expert_cache_finish_pending_batch(void) {
    return hip_stream_batch_selected_finish_pending_missing();
}
...
extern "C" int ds4_gpu_stream_expert_cache_seed_experts(
        ...
        uint32_t                           n_experts) {
    if (!table) return 0;
    return hip_stream_resident_seed_experts(table->model_map,
                                             table->model_size,
                                             table->layer,
                                             expert_ids,
                                             expert_priorities,
