...
static hipStream_t g_stream_selected_upload_stream;
static hipStream_t g_selected_readback_stream;
static hipEvent_t g_selected_readback_event;
...
struct hip_stream_selected_cache {
    int loaded;
...
struct hip_stream_resident_expert {
    const void *model_map;
    uint32_t layer;
    int32_t expert;
    uint64_t gate_expert_bytes;
 ...
 * hipMalloc/hipFree round trip per cache miss.  Allocator traffic was a
 ...
 */
struct hip_stream_expert_slab {
    char *base;
    uint64_t bytes;
};

struct hip_stream_resident_key {
    const void *model_map;
    uint32_t layer;
    int32_t expert;
    uint64_t gate_offset;
    ...
    bool operator==(const hip_stream_resident_key &o) const {
...
struct hip_stream_resident_key_hash {
    size_t operator()(const hip_stream_resident_key &k) const {
...
struct hip_stream_batch_selected_cache {
    int loaded;
    const void *model_map;
    uint32_t layer;
    uint32_t n_total_expert;
...
struct hip_stream_layer_expert_cache {
    int active;
    const void *model_map;
    uint32_t layer;
    uint32_t n_total_expert;
...
struct hip_stream_cache_layer_stats {
    uint64_t selected_calls;
    uint64_t selected_slots;
    uint64_t selected_hits;
    uint64_t selected_misses;
...
static std::vector<hip_model_image> g_model_images;
...
static hip_stream_selected_cache g_stream_selected_cache;
static hip_stream_batch_selected_cache g_stream_batch_selected_cache;
static hip_stream_layer_expert_cache g_stream_layer_expert_cache[2];
static std::vector<hip_stream_resident_expert> g_stream_resident_experts;
static std::unordered_map<hip_stream_resident_key,
                          size_t,
                          hip_stream_resident_key_hash> g_stream_resident_index;
static uint64_t g_stream_resident_bytes;
static uint64_t g_stream_resident_clock;
static std::vector<hip_stream_expert_slab> g_stream_expert_slabs;
...
static hip_stream_cache_stats g_stream_cache_stats;
static hip_stream_cache_layer_stats
...
static hipEvent_t g_stream_selected_reuse_event;
static int g_stream_selected_reuse_event_pending;
static hipEvent_t g_stream_selected_upload_ready_event;
static int g_stream_selected_upload_event_pending;
static hipEvent_t g_stream_batch_selected_reuse_event;
static int g_stream_batch_selected_reuse_event_pending;
static hipEvent_t g_stream_batch_selected_upload_ready_event;
...
static hipStream_t g_stream_read_upload_streams[DS4_ROCM_STREAM_READ_WORKERS];
...
static struct hip_stream_read_job *g_stream_read_active_jobs;
...
static int hip_model_stage_read(void *stage, uint64_t stage_bytes,
                                 ...
                                 const char **payload);
static int hip_stream_selected_reuse_wait(const char *what);
static int hip_stream_selected_upload_wait_host(const char *what);
static void hip_stream_selected_stage_release(void);
static int hip_stream_batch_selected_reuse_wait(const char *what);
static int hip_stream_batch_selected_upload_wait_host(const char *what);
static void hip_stream_batch_selected_stage_release(void);
static void hip_stream_read_pool_shutdown(void);
...
static int hip_u64_add_checked(uint64_t a, uint64_t b, uint64_t *out) {
...
static int hip_stream_cache_layer_stats_on(void) {
...
static void hip_stream_cache_stats_note_resident(void) {
    if (!hip_stream_cache_stats_on()) return;
...
static void hip_stream_cache_stats_print(const char *label) {
    if (!hip_stream_cache_stats_on()) return;
    fprintf(stderr,
    ...
    if (!hip_stream_cache_layer_stats_on()) return;
    for (uint32_t layer = 0;
         layer < DS4_ROCM_STREAM_CACHE_LAYER_STATS_MAX;
         layer++) {
        const hip_stream_cache_layer_stats *s =
...
static const char *hip_model_range_ptr_from_fd(
        ...
        uint64_t bytes,
        const char *what);
static const char *hip_model_range_ptr(
        const void *model_map,
        uint64_t offset,
        uint64_t bytes,
        const char *what);
static int hip_model_range_is_cached(
        const void *model_map,
        uint64_t offset,
        uint64_t bytes);
...
static int hip_model_image_find(const void *model_map) {
...
static const char *hip_model_image_ptr(const void *model_map, uint64_t offset) {
    for (const hip_model_image &img : g_model_images) {
...
static const char *hip_model_image_range_ptr(
        const void *model_map,
        uint64_t    offset,
        uint64_t    bytes) {
    if (bytes == 0) return hip_model_image_ptr(model_map, offset);
    for (const hip_model_image &img : g_model_images) {
        if (img.host_base != model_map || offset < img.device_offset) continue;
        const uint64_t rel = offset - img.device_offset;
...
static int hip_model_image_owned(const void *model_map) {
    return hip_model_image_find(model_map) >= 0;
}

static uint64_t hip_model_image_bytes(void) {
    uint64_t bytes = 0;
    for (const hip_model_image &img : g_model_images) bytes += img.size;
    return bytes;
}

static void hip_model_image_release_all(void) {
    for (const hip_model_image &img : g_model_images) {
        if (img.device_ptr) (void)hipFree(img.device_ptr);
    }
    g_model_images.clear();
}

static int hip_stream_resident_reclaim_wait(const char *what) {
    if (!hip_stream_selected_reuse_wait(what)) return 0;
    if (!hip_stream_batch_selected_reuse_wait(what)) return 0;
    return 1;
}

static void hip_stream_resident_cache_release(void) {
    if (!hip_stream_resident_reclaim_wait(
                "streaming resident expert cache release")) {
        return;
    }
    for (hip_stream_resident_expert &e : g_stream_resident_experts) {
        if (e.base && !e.pooled) (void)hipFree(e.base);
    }
    g_stream_resident_experts.clear();
    g_stream_resident_index.clear();
    g_stream_resident_bytes = 0;
    g_stream_resident_clock = 0;
    for (hip_stream_expert_slab &slab : g_stream_expert_slabs) {
        if (slab.base) (void)hipFree(slab.base);
    }
...
static void hip_stream_layer_expert_cache_release(void) {
    ...
    for (uint32_t i = 0; i < 2u; i++) {
        hip_stream_layer_expert_cache &c = g_stream_layer_expert_cache[i];
        if (c.base) (void)hipFree(c.base);
        memset(&c, 0, sizeof(c));
    }
}

static void hip_stream_read_stage_release(void) {
    hip_stream_read_pool_shutdown();
    for (uint32_t i = 0; i < DS4_ROCM_STREAM_READ_WORKERS; i++) {
        if (g_stream_read_stage_raw[i]) {
            (void)hipHostFree(g_stream_read_stage_raw[i]);
    ...
    for (uint32_t i = 0; i < DS4_ROCM_STREAM_READ_WORKERS; i++) {
        if (g_stream_read_upload_streams[i]) {
            (void)hipStreamDestroy(g_stream_read_upload_streams[i]);
            g_stream_read_upload_streams[i] = NULL;
        }
    }
}

static void hip_stream_batch_selected_cache_release(void) {
    (void)hip_stream_batch_selected_reuse_wait(
            "streaming batch selected cache release");
    (void)hip_stream_batch_selected_upload_wait_host(
            "streaming batch selected cache release upload");
    if (g_stream_batch_selected_cache.selected_ids) {
        (void)hipFree(g_stream_batch_selected_cache.selected_ids);
    }
    if (g_stream_batch_selected_cache.pair_missing) {
        (void)hipFree(g_stream_batch_selected_cache.pair_missing);
    }
    if (g_stream_batch_selected_cache.gate_ptrs) {
        (void)hipFree(g_stream_batch_selected_cache.gate_ptrs);
    }
    if (g_stream_batch_selected_cache.up_ptrs) {
        (void)hipFree(g_stream_batch_selected_cache.up_ptrs);
    }
    if (g_stream_batch_selected_cache.down_ptrs) {
        (void)hipFree(g_stream_batch_selected_cache.down_ptrs);
    }
    if (g_stream_batch_selected_cache.resident_gate_ptrs) {
        (void)hipFree(g_stream_batch_selected_cache.resident_gate_ptrs);
    }
    if (g_stream_batch_selected_cache.resident_up_ptrs) {
        (void)hipFree(g_stream_batch_selected_cache.resident_up_ptrs);
    }
    if (g_stream_batch_selected_cache.missing_gate_ptrs) {
        (void)hipFree(g_stream_batch_selected_cache.missing_gate_ptrs);
    }
    if (g_stream_batch_selected_cache.missing_up_ptrs) {
        (void)hipFree(g_stream_batch_selected_cache.missing_up_ptrs);
    }
    if (g_stream_batch_selected_reuse_event) {
        (void)hipEventDestroy(g_stream_batch_selected_reuse_event);
        g_stream_batch_selected_reuse_event = NULL;
    }
    if (g_stream_batch_selected_upload_ready_event) {
        (void)hipEventDestroy(g_stream_batch_selected_upload_ready_event);
    ...
    hip_stream_batch_selected_stage_release();
    memset(&g_stream_batch_selected_cache, 0, sizeof(g_stream_batch_selected_cache));
}

static void hip_stream_selected_cache_release(void) {
    (void)hip_stream_selected_reuse_wait("streaming selected cache release");
    (void)hip_stream_selected_upload_wait_host(
            "streaming selected cache release upload");
    if (g_stream_selected_cache.gate) (void)hipFree(g_stream_selected_cache.gate);
    if (g_stream_selected_cache.up) (void)hipFree(g_stream_selected_cache.up);
    if (g_stream_selected_cache.down) (void)hipFree(g_stream_selected_cache.down);
    if (g_stream_selected_cache.slot_ids) (void)hipFree(g_stream_selected_cache.slot_ids);
    if (g_stream_selected_cache.gate_ptrs) (void)hipFree(g_stream_selected_cache.gate_ptrs);
    if (g_stream_selected_cache.up_ptrs) (void)hipFree(g_stream_selected_cache.up_ptrs);
    if (g_stream_selected_cache.down_ptrs) (void)hipFree(g_stream_selected_cache.down_ptrs);
    if (g_stream_selected_reuse_event) {
        (void)hipEventDestroy(g_stream_selected_reuse_event);
        g_stream_selected_reuse_event = NULL;
    }
    if (g_stream_selected_upload_ready_event) {
        (void)hipEventDestroy(g_stream_selected_upload_ready_event);
        g_stream_selected_upload_ready_event = NULL;
    }
    g_stream_selected_reuse_event_pending = 0;
    g_stream_selected_upload_event_pending = 0;
    hip_stream_selected_stage_release();
    memset(&g_stream_selected_cache, 0, sizeof(g_stream_selected_cache));
    hip_stream_batch_selected_cache_release();
    hip_stream_resident_cache_release();
    hip_stream_layer_expert_cache_release();
    hip_stream_read_stage_release();
    g_routed_moe_selected_override_n = 0;
}

static int hip_stream_selected_ensure_stream(void) {
    if (g_stream_selected_upload_stream) return 1;
    hipError_t err = hipStreamCreateWithFlags(&g_stream_selected_upload_stream, hipStreamNonBlocking);
    if (err != hipSuccess) {
...
static int hip_stream_selected_reuse_ensure_event(void) {
    if (g_stream_selected_reuse_event) return 1;
    hipError_t err =
        hipEventCreateWithFlags(&g_stream_selected_reuse_event,
...
static int hip_stream_selected_reuse_wait(const char *what) {
    ...
    hipError_t err = hipEventSynchronize(g_stream_selected_reuse_event);
    if (err != hipSuccess) {
        fprintf(stderr,
...
static int hip_stream_selected_upload_ensure_event(void) {
    if (g_stream_selected_upload_ready_event) return 1;
    hipError_t err =
        hipEventCreateWithFlags(&g_stream_selected_upload_ready_event,
                                 hipEventDisableTiming);
    if (err != hipSuccess) {
        fprintf(stderr,
...
static int hip_stream_selected_upload_wait_host(const char *what) {
    ...
    hipError_t err = hipEventSynchronize(g_stream_selected_upload_ready_event);
    if (err != hipSuccess) {
        fprintf(stderr,
                DS4_GPU_LOG_PREFIX "%s wait failed: %s\n",
...
static int hip_stream_selected_upload_record_ready(void) {
    if (!hip_stream_selected_upload_ensure_event()) return 0;
    hipError_t err = hipEventRecord(g_stream_selected_upload_ready_event,
...
static int hip_stream_selected_wait_upload_ready(void) {
    if (!g_stream_selected_upload_event_pending) return 1;
#ifdef __HIP_PLATFORM_AMD__
    hipError_t err =
...
static int hip_stream_selected_mark_inflight(void) {
    if (!g_ssd_streaming_mode) return 1;
    if (!hip_stream_selected_reuse_ensure_event()) return 0;
    hipError_t err = hipEventRecord(g_stream_selected_reuse_event, 0);
    if (err != hipSuccess) {
        fprintf(stderr,
...
static int hip_stream_batch_selected_reuse_ensure_event(void) {
    if (g_stream_batch_selected_reuse_event) return 1;
    hipError_t err =
        hipEventCreateWithFlags(&g_stream_batch_selected_reuse_event,
                                 hipEventDisableTiming);
    if (err != hipSuccess) {
        fprintf(stderr,
...
static int hip_stream_batch_selected_upload_ensure_event(void) {
    if (g_stream_batch_selected_upload_ready_event) return 1;
    hipError_t err =
        hipEventCreateWithFlags(&g_stream_batch_selected_upload_ready_event,
                                 hipEventDisableTiming);
    if (err != hipSuccess) {
        fprintf(stderr,
...
static int hip_stream_batch_selected_upload_wait_host(const char *what) {
    ...
    hipError_t err =
        hipEventSynchronize(g_stream_batch_selected_upload_ready_event);
    if (err != hipSuccess) {
        fprintf(stderr,
                DS4_GPU_LOG_PREFIX "%s wait failed: %s\n",
...
static int hip_stream_batch_selected_upload_record_ready(void) {
    if (!hip_stream_batch_selected_upload_ensure_event()) return 0;
    hipError_t err = hipEventRecord(g_stream_batch_selected_upload_ready_event,
                                      g_stream_selected_upload_stream);
    if (err != hipSuccess) {
        fprintf(stderr,
...
static int hip_stream_batch_selected_wait_upload_ready(void) {
    if (!g_stream_batch_selected_upload_event_pending) return 1;
#ifdef __HIP_PLATFORM_AMD__
    hipError_t err =
...
static int hip_stream_batch_selected_reuse_wait(const char *what) {
    ...
    hipError_t err = hipEventSynchronize(g_stream_batch_selected_reuse_event);
    if (err != hipSuccess) {
        fprintf(stderr,
                DS4_GPU_LOG_PREFIX "%s wait failed: %s\n",
...
static int hip_stream_batch_selected_mark_inflight(void) {
    if (!g_ssd_streaming_mode) return 1;
    if (!hip_stream_batch_selected_reuse_ensure_event()) return 0;
    hipError_t err = hipEventRecord(g_stream_batch_selected_reuse_event, 0);
    if (err != hipSuccess) {
        fprintf(stderr,
...
static void hip_stream_selected_stage_release(void) {
    if (g_stream_selected_cache.gate_ptrs_stage) {
        (void)hipHostFree((void *)g_stream_selected_cache.gate_ptrs_stage);
    }
    if (g_stream_selected_cache.up_ptrs_stage) {
        (void)hipHostFree((void *)g_stream_selected_cache.up_ptrs_stage);
    }
    if (g_stream_selected_cache.down_ptrs_stage) {
        (void)hipHostFree((void *)g_stream_selected_cache.down_ptrs_stage);
    }
...
static int hip_stream_selected_ensure_buffers(uint64_t gate_bytes, uint64_t down_bytes) {
    if (gate_bytes == 0 || down_bytes == 0) return 0;
    if (!hip_stream_selected_upload_wait_host(
    ...
    if (g_stream_selected_cache.gate_capacity < gate_bytes) {
        ...
        err = hipMalloc((void **)&g_stream_selected_cache.gate, (size_t)gate_bytes);
        if (err == hipSuccess) {
            err = hipMalloc((void **)&g_stream_selected_cache.up, (size_t)gate_bytes);
        }
        if (err != hipSuccess) {
    ...
    if (g_stream_selected_cache.down_capacity < down_bytes) {
        ...
        err = hipMalloc((void **)&g_stream_selected_cache.down, (size_t)down_bytes);
        if (err != hipSuccess) {
    ...
    if (!g_stream_selected_cache.slot_ids) {
        err = hipMalloc((void **)&g_stream_selected_cache.slot_ids,
        ...
        err = hipMemcpy(g_stream_selected_cache.slot_ids,
                         slots,
                         sizeof(slots),
    ...
    if (!g_stream_selected_cache.gate_ptrs) {
        err = hipMalloc((void **)&g_stream_selected_cache.gate_ptrs,
                         DS4_ROCM_N_EXPERT_USED * sizeof(char *));
        if (err == hipSuccess) {
            err = hipMalloc((void **)&g_stream_selected_cache.up_ptrs,
                             DS4_ROCM_N_EXPERT_USED * sizeof(char *));
        }
        if (err == hipSuccess) {
            err = hipMalloc((void **)&g_stream_selected_cache.down_ptrs,
                             DS4_ROCM_N_EXPERT_USED * sizeof(char *));
        }
        if (err != hipSuccess) {
    ...
    if (!g_stream_selected_cache.gate_ptrs_stage) {
        void *stage = NULL;
        err = hipHostMalloc(&stage, DS4_ROCM_N_EXPERT_USED * sizeof(char *));
        g_stream_selected_cache.gate_ptrs_stage =
            err == hipSuccess ? (const char **)stage : NULL;
        stage = NULL;
        if (err == hipSuccess) {
            err = hipHostMalloc(&stage, DS4_ROCM_N_EXPERT_USED * sizeof(char *));
...
static void hip_stream_batch_selected_stage_release(void) {
    if (g_stream_batch_selected_cache.selected_stage) {
        (void)hipHostFree(g_stream_batch_selected_cache.selected_stage);
    }
    if (g_stream_batch_selected_cache.pair_missing_stage) {
        (void)hipHostFree(g_stream_batch_selected_cache.pair_missing_stage);
    }
    if (g_stream_batch_selected_cache.gate_ptrs_stage) {
        (void)hipHostFree((void *)g_stream_batch_selected_cache.gate_ptrs_stage);
    }
    if (g_stream_batch_selected_cache.up_ptrs_stage) {
        (void)hipHostFree((void *)g_stream_batch_selected_cache.up_ptrs_stage);
    }
    if (g_stream_batch_selected_cache.down_ptrs_stage) {
        (void)hipHostFree((void *)g_stream_batch_selected_cache.down_ptrs_stage);
    }
    if (g_stream_batch_selected_cache.resident_gate_ptrs_stage) {
        (void)hipHostFree((void *)g_stream_batch_selected_cache.resident_gate_ptrs_stage);
    }
    if (g_stream_batch_selected_cache.resident_up_ptrs_stage) {
        (void)hipHostFree((void *)g_stream_batch_selected_cache.resident_up_ptrs_stage);
    }
    if (g_stream_batch_selected_cache.missing_gate_ptrs_stage) {
        (void)hipHostFree((void *)g_stream_batch_selected_cache.missing_gate_ptrs_stage);
    }
    if (g_stream_batch_selected_cache.missing_up_ptrs_stage) {
        (void)hipHostFree((void *)g_stream_batch_selected_cache.missing_up_ptrs_stage);
    }
...
static int hip_stream_batch_selected_ensure_buffers(
        uint64_t n_ids,
        uint32_t n_unique) {
    if (n_ids == 0 || n_unique == 0) return 0;
    if (!hip_stream_batch_selected_reuse_wait(
                "streaming batch selected cache reuse")) {
        return 0;
    }
    if (!hip_stream_batch_selected_upload_wait_host(
    ...
    if (g_stream_batch_selected_cache.selected_capacity < selected_bytes) {
        ...
        err = hipMalloc((void **)&g_stream_batch_selected_cache.selected_ids,
    ...
    if (g_stream_batch_selected_cache.pair_missing_capacity < n_ids) {
        ...
        err = hipMalloc((void **)&g_stream_batch_selected_cache.pair_missing,
                         (size_t)n_ids);
        if (err != hipSuccess) {
            fprintf(stderr,
    ...
    if (g_stream_batch_selected_cache.ptr_capacity < n_unique) {
        ...
        err = hipMalloc((void **)&g_stream_batch_selected_cache.gate_ptrs,
                         (size_t)n_unique * sizeof(char *));
        if (err == hipSuccess) {
            err = hipMalloc((void **)&g_stream_batch_selected_cache.up_ptrs,
                             (size_t)n_unique * sizeof(char *));
        }
        if (err == hipSuccess) {
            err = hipMalloc((void **)&g_stream_batch_selected_cache.down_ptrs,
                             (size_t)n_unique * sizeof(char *));
        }
        if (err == hipSuccess) {
            err = hipMalloc((void **)&g_stream_batch_selected_cache.resident_gate_ptrs,
                             (size_t)n_unique * sizeof(char *));
        }
        if (err == hipSuccess) {
            err = hipMalloc((void **)&g_stream_batch_selected_cache.resident_up_ptrs,
                             (size_t)n_unique * sizeof(char *));
        }
        if (err == hipSuccess) {
            err = hipMalloc((void **)&g_stream_batch_selected_cache.missing_gate_ptrs,
                             (size_t)n_unique * sizeof(char *));
        }
        if (err == hipSuccess) {
            err = hipMalloc((void **)&g_stream_batch_selected_cache.missing_up_ptrs,
                             (size_t)n_unique * sizeof(char *));
        }
        if (err != hipSuccess) {
            fprintf(stderr,
    ...
    if (g_stream_batch_selected_cache.selected_stage_capacity < selected_bytes ||
        ...
        hip_stream_batch_selected_stage_release();
        err = hipHostMalloc((void **)&g_stream_batch_selected_cache.selected_stage,
                             (size_t)selected_bytes);
        if (err == hipSuccess) {
            err = hipHostMalloc((void **)&g_stream_batch_selected_cache.pair_missing_stage,
                                 (size_t)n_ids);
        }
        void *stage = NULL;
        if (err == hipSuccess) {
            err = hipHostMalloc(&stage, (size_t)n_unique * sizeof(char *));
...
static int hip_stream_selected_is_current(
        const hip_stream_resident_expert &e,
        uint32_t layer,
...
static hip_stream_resident_key hip_stream_resident_make_key(
        const void *model_map,
        uint32_t layer,
        int32_t expert,
        uint64_t gate_offset,
...
static hip_stream_resident_key hip_stream_resident_entry_key(
        const hip_stream_resident_expert &e) {
    return hip_stream_resident_make_key(e.model_map,
                                         e.layer,
                                         e.expert,
                                         e.gate_offset,
                                         e.up_offset,
...
static int hip_stream_resident_find(
        const void *model_map,
        uint32_t layer,
        int32_t expert,
        uint64_t gate_offset,
        ...
        uint64_t gate_expert_bytes,
        uint64_t down_expert_bytes) {
    const hip_stream_resident_key key =
        hip_stream_resident_make_key(model_map,
                                      layer,
                                      expert,
                                      gate_offset,
                                      up_offset,
...
static int hip_stream_resident_evict_at(size_t idx) {
    if (idx >= g_stream_resident_experts.size()) return 0;
    if (!hip_stream_resident_reclaim_wait(
                "streaming resident expert cache eviction")) {
        return 0;
    }
    hip_stream_resident_expert &e = g_stream_resident_experts[idx];
    const hip_stream_resident_key evicted_key =
        hip_stream_resident_entry_key(e);
    if (hip_stream_cache_stats_on()) {
    ...
    if (idx != last) {
        ...
        g_stream_resident_index[hip_stream_resident_entry_key(
...
static int hip_stream_resident_evict_one(
        uint32_t layer,
        const int32_t *selected_ids,
        uint32_t n_selected) {
    size_t victim = (size_t)-1;
    uint64_t oldest = UINT64_MAX;
    if (hip_stream_evict_past_layers_first()) {
        for (size_t i = 0; i < g_stream_resident_experts.size(); i++) {
            const hip_stream_resident_expert &e =
                g_stream_resident_experts[i];
            if (e.layer > layer ||
                hip_stream_selected_is_current(e,
                                                layer,
                                                selected_ids,
                                                n_selected)) {
                continue;
        ...
        if (victim != (size_t)-1) {
            return hip_stream_resident_evict_at(victim);
        }
    }
    oldest = UINT64_MAX;
    for (size_t i = 0; i < g_stream_resident_experts.size(); i++) {
        const hip_stream_resident_expert &e = g_stream_resident_experts[i];
        if (hip_stream_selected_is_current(e, layer, selected_ids, n_selected)) {
            continue;
        }
        if (e.last_used < oldest) {
            oldest = e.last_used;
...
static uint64_t hip_stream_resident_free_reserve_bytes(void) {
    /*
...
static int hip_stream_resident_make_room(
        uint64_t bytes,
        uint32_t layer,
        const int32_t *selected_ids,
        uint32_t n_selected) {
    while (g_stream_resident_experts.size() >= g_stream_expert_cache_budget) {
        if (!hip_stream_resident_evict_one(layer, selected_ids, n_selected)) {
            break;
        }
    }
    ...
    size_t free_b = 0;
    size_t total_b = 0;
    const uint64_t reserve = hip_stream_resident_free_reserve_bytes();
    while (hipMemGetInfo(&free_b, &total_b) == hipSuccess) {
        (void)total_b;
...
static int hip_stream_expert_slab_grow(uint64_t slot_bytes) {
    ...
    while (slab_slots != 0) {
        ...
        if (!hip_u64_mul_checked(slab_slots, slot_bytes, &slab_bytes)) {
            slab_slots >>= 1u;
            continue;
        }
        size_t free_b = 0;
        ...
        void *base = NULL;
        hipError_t err = hipMalloc(&base, (size_t)slab_bytes);
        if (err != hipSuccess) {
            (void)hipGetLastError();
            slab_slots >>= 1u;
            continue;
...
static char *hip_stream_expert_slot_acquire(
        uint64_t bytes,
        uint32_t layer,
        const int32_t *selected_ids,
        uint32_t n_selected) {
    ...
    for (;;) {
        ...
        if (g_stream_expert_slot_count < g_stream_expert_cache_budget &&
            hip_stream_expert_slab_grow(bytes)) {
            continue;
        }
        if (!hip_stream_resident_evict_one(layer, selected_ids, n_selected)) {
            return NULL;
...
static int hip_stream_resident_alloc(
        const void *model_map,
        uint32_t layer,
        int32_t expert,
        const int32_t *selected_ids,
    ...
    if (!hip_u64_mul_checked(2u, gate_expert_bytes, &gate_pair) ||
        gate_pair > UINT64_MAX - down_expert_bytes) {
        return -1;
    }
    bytes = gate_pair + down_expert_bytes;

    void *base = hip_stream_expert_slot_acquire(bytes,
                                                 layer,
                                                 selected_ids,
                                                 n_selected);
    ...
    if (!pooled) {
         ...
         */
        if (!hip_stream_resident_make_room(bytes, layer, selected_ids, n_selected)) {
            fprintf(stderr,
                    ...
                    layer,
                    expert,
                    (double)hip_stream_resident_free_reserve_bytes() / 1073741824.0);
            return -1;
        }

        hipError_t err = hipMalloc(&base, (size_t)bytes);
        while (err != hipSuccess && hip_stream_resident_evict_one(layer, selected_ids, n_selected)) {
            (void)hipGetLastError();
            err = hipMalloc(&base, (size_t)bytes);
        }
        if (err != hipSuccess) {
            fprintf(stderr,
    ...
    g_stream_resident_index[hip_stream_resident_entry_key(e)] =
    ...
    hip_stream_cache_stats_note_resident();
    return (int)g_stream_resident_experts.size() - 1;
}

typedef struct hip_stream_read_job {
    char *dst;
    uint64_t offset;
    uint64_t bytes;
    void *host_raw;
...
struct hip_stream_batch_selected_pending {
    int active;
    const void *model_map;
    uint32_t layer;
    uint32_t n_total_expert;
    ...
    uint64_t gate_expert_bytes;
    uint64_t down_expert_bytes;
    hip_stream_read_job read_jobs[DS4_ROCM_STREAM_READ_MAX_JOBS];
    uint32_t read_job_count;
};

static hip_stream_batch_selected_pending g_stream_batch_selected_pending;

struct hip_stream_selected_pending {
    int active;
    const void *model_map;
    uint32_t layer;
    uint32_t n_total_expert;
    ...
    int32_t selected_ids[DS4_ROCM_N_EXPERT_USED];
    hip_stream_read_job read_jobs[DS4_ROCM_N_EXPERT_USED * 3u];
    uint32_t read_job_count;
};

static hip_stream_selected_pending g_stream_selected_pending;

typedef struct hip_stream_read_profile_span {
    uint64_t offset;
    uint64_t end;
} hip_stream_read_profile_span;

static uint64_t hip_stream_read_profile_us(double seconds) {
    return seconds <= 0.0 ? 0u : (uint64_t)(seconds * 1000000.0 + 0.5);
}

static void hip_stream_read_profile_coalesce(
        hip_stream_read_profile_span *spans,
        uint32_t count,
        uint64_t max_gap,
        uint64_t *groups_out,
        uint64_t *extra_out) {
...
static void hip_stream_read_profile_note_jobs(
        const hip_stream_read_job *jobs,
        uint32_t count) {
    if (g_stream_read_profile_enabled != 1 || !jobs || count == 0) return;
    if (count > DS4_ROCM_STREAM_READ_MAX_JOBS) return;

    hip_stream_read_profile_span spans[DS4_ROCM_STREAM_READ_MAX_JOBS];
    uint32_t n = 0;
    for (uint32_t i = 0; i < count; i++) {
    ...
    for (uint32_t i = 1; i < n; i++) {
        const hip_stream_read_profile_span x = spans[i];
        uint32_t j = i;
    ...
    hip_stream_read_profile_coalesce(spans, n, 0,
                                      &exact_groups, &exact_extra);
    hip_stream_read_profile_coalesce(spans, n, 1ull * 1048576ull,
                                      &gap1m_groups, &gap1m_extra);
    hip_stream_read_profile_coalesce(spans, n, 4ull * 1048576ull,
...
static void hip_stream_read_profile_print(void) {
...
static int hip_stream_read_profile_enabled(void) {
    if (g_stream_read_profile_enabled < 0) {
        ...
        if (g_stream_read_profile_enabled &&
            ...
            atexit(hip_stream_read_profile_print);
...
static int hip_stream_read_direct_disabled(void) {
...
static void hip_stream_read_job_run(hip_stream_read_job *job,
                                     void *stage,
     ...
     */
    if (!hip_stream_read_direct_disabled() &&
        g_model_direct_fd >= 0 &&
        g_model_direct_align > 1 &&
        g_model_file_size != 0) {
        const uint64_t aligned_off =
            hip_round_down(job->offset, g_model_direct_align);
        const uint64_t delta = job->offset - aligned_off;
        const uint64_t read_size =
            hip_round_up(delta + job->bytes, g_model_direct_align);
        if (read_size <= stage_bytes &&
            aligned_off <= g_model_file_size &&
            read_size <= g_model_file_size - aligned_off &&
            hip_pread_full(g_model_direct_fd, stage, read_size, aligned_off)) {
...
#endif
    if (hip_pread_full(g_model_fd, job->host_buf, job->bytes, job->offset)) {
        job->ok = 1;
    } else {
...
static int hip_stream_read_job_upload(
        hip_stream_read_job *job,
        hipStream_t stream) {
    if (!job || !job->ok || !job->dst || !job->host_buf || !stream) {
        if (job) job->errnum = EINVAL;
        return 0;
    }
    hipError_t err = hipMemcpyAsync(job->dst,
                                      job->host_buf,
                                      (size_t)job->bytes,
                                      hipMemcpyHostToDevice,
                                      stream);
    ...
    job->uploaded = 1;
    if (!job->direct) hip_model_drop_file_pages(job->offset, job->bytes);
    return 1;
}

static void *hip_stream_read_worker(void *arg) {
    ...
    for (;;) {
        ...
        hip_stream_read_job *job = &g_stream_read_active_jobs[idx];
        void *stage = NULL;
        ...
        const double read_t0 = profile ? hip_wall_sec() : 0.0;
        hip_stream_read_job_run(job, stage, stage_bytes);
        uint64_t read_us = 0;
        uint64_t upload_us = 0;
        if (profile) {
            read_us = hip_stream_read_profile_us(hip_wall_sec() - read_t0);
        }
        if (job->ok) {
            const double upload_t0 = profile ? hip_wall_sec() : 0.0;
            (void)hip_stream_read_job_upload(
                    job,
                    worker_id < DS4_ROCM_STREAM_READ_WORKERS ?
                        g_stream_read_upload_streams[worker_id] : NULL);
            if (profile) {
                upload_us =
                    hip_stream_read_profile_us(hip_wall_sec() - upload_t0);
            }
        }
...
static uint32_t hip_stream_read_worker_count(void) {
...
static void hip_stream_read_upload_streams_destroy(void) {
    for (uint32_t i = 0; i < DS4_ROCM_STREAM_READ_WORKERS; i++) {
        if (g_stream_read_upload_streams[i]) {
            (void)hipStreamDestroy(g_stream_read_upload_streams[i]);
            g_stream_read_upload_streams[i] = NULL;
...
static int hip_stream_read_upload_streams_ensure(void) {
    const uint32_t workers = hip_stream_read_worker_count();
    for (uint32_t i = 0; i < workers; i++) {
        if (g_stream_read_upload_streams[i]) continue;
        hipError_t err = hipStreamCreateWithFlags(
        ...
        if (err != hipSuccess) {
            fprintf(stderr,
                    ...
                    hipGetErrorString(err));
            (void)hipGetLastError();
            hip_stream_read_upload_streams_destroy();
            return 0;
        }
    }
    return 1;
...
static int hip_stream_read_pool_ensure(void) {
    ...
    g_stream_read_pool_workers = hip_stream_read_worker_count();
    (void)hip_stream_read_profile_enabled();
    if (!hip_stream_read_upload_streams_ensure()) {
        pthread_mutex_unlock(&g_stream_read_mutex);
        return 0;
    }
...
static int hip_stream_read_jobs_prepare(hip_stream_read_job *jobs, uint32_t count) {
    ...
    for (uint32_t i = 0; i < workers; i++) {
        if (g_stream_read_stage_bytes[i] < max_bytes) {
            ...
            hipError_t err = hipHostMalloc(&g_stream_read_stage_raw[i],
                                             (size_t)max_bytes);
            if (err != hipSuccess) {
                fprintf(stderr,
...
static int hip_stream_read_jobs_start(hip_stream_read_job *jobs, uint32_t count) {
    if (!jobs || count == 0) return 1;
    if (!hip_stream_read_pool_ensure()) return 0;
    ...
    if (!hip_stream_read_jobs_prepare(jobs, count)) {
        pthread_mutex_unlock(&g_stream_read_mutex);
        return 0;
    }
    hip_stream_read_profile_note_jobs(jobs, count);
...
static int hip_stream_read_jobs_wait(hip_stream_read_job *jobs, uint32_t count) {
    if (!jobs || count == 0) return 1;
    const int profile = g_stream_read_profile_enabled == 1;
    const double wait_t0 = profile ? hip_wall_sec() : 0.0;
    pthread_mutex_lock(&g_stream_read_mutex);
    ...
    if (profile) {
        ...
        g_stream_read_profile_wait_us +=
            hip_stream_read_profile_us(hip_wall_sec() - wait_t0);
    }
...
static int hip_stream_read_jobs_parallel(hip_stream_read_job *jobs, uint32_t count) {
    if (!jobs || count == 0) return 1;
    return hip_stream_read_jobs_start(jobs, count) &&
           hip_stream_read_jobs_wait(jobs, count);
}

static void hip_stream_read_jobs_free(hip_stream_read_job *jobs, uint32_t count) {
    if (!jobs) return;
    for (uint32_t i = 0; i < count; i++) {
        jobs[i].host_raw = NULL;
        jobs[i].host_buf = NULL;
...
static int hip_stream_selected_upload_read_jobs(
        hip_stream_read_job *jobs,
        uint32_t count);

static int hip_stream_batch_selected_pending_matches(
        const void *model_map,
        uint32_t layer,
        uint32_t n_total_expert,
        uint32_t n_selected,
...
static int hip_stream_batch_selected_apply_split(
        const void *model_map,
        uint32_t layer,
        uint32_t n_total_expert,
        uint32_t n_selected,
    ...
    if (!selected_exec || !resident_gate_ptrs || !resident_up_ptrs ||
        ...
        !hip_stream_batch_selected_pending_matches(model_map,
                                                    layer,
                                                    n_total_expert,
                                                    n_selected,
                                                    n_tokens,
...
static int hip_stream_batch_selected_finish_pending_missing(void) {
    ...
    if (!hip_stream_read_jobs_wait(g_stream_batch_selected_pending.read_jobs,
                                    read_job_count) ||
        !hip_stream_selected_upload_read_jobs(
                g_stream_batch_selected_pending.read_jobs,
                read_job_count)) {
        hip_stream_read_jobs_free(g_stream_batch_selected_pending.read_jobs,
                                   read_job_count);
...
static void hip_stream_batch_selected_abort_pending(void) {
    if (!g_stream_batch_selected_pending.active) return;
    const uint32_t read_job_count =
        g_stream_batch_selected_pending.read_job_count;
    (void)hip_stream_read_jobs_wait(g_stream_batch_selected_pending.read_jobs,
                                     read_job_count);
    hip_stream_read_jobs_free(g_stream_batch_selected_pending.read_jobs,
                               read_job_count);
    memset(&g_stream_batch_selected_pending, 0,
...
static void hip_stream_selected_abort_pending(void) {
    if (!g_stream_selected_pending.active) return;
    const uint32_t read_job_count = g_stream_selected_pending.read_job_count;
    if (read_job_count != 0) {
        (void)hip_stream_read_jobs_wait(g_stream_selected_pending.read_jobs,
                                         read_job_count);
        hip_stream_read_jobs_free(g_stream_selected_pending.read_jobs,
                                   read_job_count);
    }
...
static int hip_stream_selected_upload_read_jobs(
    ...
    if (!hip_stream_selected_ensure_stream()) return 0;
    for (uint32_t i = 0; i < count; i++) {
        if (jobs[i].uploaded) continue;
        hipError_t err = hipMemcpyAsync(jobs[i].dst,
                                          jobs[i].host_buf,
                                          (size_t)jobs[i].bytes,
                                          hipMemcpyHostToDevice,
                                          g_stream_selected_upload_stream);
        ...
        if (!jobs[i].direct) {
            hip_model_drop_file_pages(jobs[i].offset, jobs[i].bytes);
        }
    }
    hipError_t err = hipStreamSynchronize(g_stream_selected_upload_stream);
    if (err != hipSuccess) {
...
static int hip_stream_flush_read_jobs(
        hip_stream_read_job *jobs,
        uint32_t *count) {
    if (!count || *count == 0) return 1;
    if (!hip_stream_read_jobs_parallel(jobs, *count) ||
        !hip_stream_selected_upload_read_jobs(jobs, *count)) {
        hip_stream_read_jobs_free(jobs, *count);
        *count = 0;
        return 0;
    }
    hip_stream_read_jobs_free(jobs, *count);
...
static int hip_stream_layer_expert_cache_apply(
        const void *model_map,
        uint32_t layer,
        uint32_t n_total_expert,
        uint64_t gate_offset,
    ...
    for (uint32_t i = 0; i < 2u; i++) {
        const hip_stream_layer_expert_cache &c = g_stream_layer_expert_cache[i];
        if (c.active &&
...
static int hip_stream_layer_expert_cache_load(
        const void *model_map,
        uint64_t model_size,
        uint32_t layer,
        uint32_t n_total_expert,
    ...
    if (!hip_u64_mul_checked(n_total_expert, gate_expert_bytes, &gate_bytes) ||
        !hip_u64_mul_checked(n_total_expert, down_expert_bytes, &down_bytes) ||
        !hip_u64_mul_checked(2u, gate_bytes, &gate_pair_bytes) ||
    ...
    if (slot.capacity < total_bytes) {
        ...
        hip_stream_resident_cache_release();
        void *base = NULL;
        hipError_t err = hipMalloc(&base, (size_t)total_bytes);
        if (err != hipSuccess) {
            fprintf(stderr,
    ...
    hip_stream_read_job *jobs =
        (hip_stream_read_job *)calloc((size_t)read_job_count, sizeof(jobs[0]));
    if (!jobs) return 0;
    ...
    if (j != read_job_count ||
        !hip_stream_read_jobs_parallel(jobs, read_job_count)) {
        ok = 0;
    }
    hip_stream_read_jobs_free(jobs, read_job_count);
    free(jobs);
    if (!ok) return 0;
...
static int hip_stream_resident_seed_experts(
        const void *model_map,
        uint64_t model_size,
        uint32_t layer,
    ...
    const int use_fd =
        g_model_fd >= 0 &&
        (g_model_fd_host_base == NULL || model_map == g_model_fd_host_base);
    if (!use_fd && !hip_stream_selected_ensure_stream()) return 1;
    ...
    for (uint32_t ri = 0; ok && ri < chosen_count; ri++) {
        ...
        int idx = hip_stream_resident_find(model_map,
                                            layer,
                                            expert_i,
                                            gate_offset,
                                            up_offset,
        ...
        idx = hip_stream_resident_alloc(model_map,
                                         layer,
                                         expert_i,
                                         protected_ids,
                                         chosen_count,
        ...
        uint64_t gate_rel = 0;
        uint64_t down_rel = 0;
        if (!hip_u64_mul_checked(expert, gate_expert_bytes, &gate_rel) ||
            !hip_u64_mul_checked(expert, down_expert_bytes, &down_rel)) {
            ok = 0;
            break;
        }

        hip_stream_resident_expert &entry =
        ...
        if (use_fd) {
            ...
            hipError_t err = hipMemcpyAsync(entry.gate,
                                              (const char *)model_map + gate_offset + gate_rel,
                                              (size_t)gate_expert_bytes,
                                              hipMemcpyHostToDevice,
                                              g_stream_selected_upload_stream);
            if (err == hipSuccess) {
                err = hipMemcpyAsync(entry.up,
    ...
    if (ok && use_fd && read_job_count != 0) {
        ok = hip_stream_read_jobs_parallel(read_jobs, read_job_count) &&
             hip_stream_selected_upload_read_jobs(read_jobs, read_job_count);
        hip_stream_read_jobs_free(read_jobs, read_job_count);
    } else if (ok && !use_fd) {
        hipError_t err = hipStreamSynchronize(g_stream_selected_upload_stream);
        if (err != hipSuccess) {
            fprintf(stderr,
...
static void hip_stream_selected_cache_header(
        const void *model_map,
        uint32_t layer,
        uint32_t n_total_expert,
        uint32_t n_selected,
...
static int hip_stream_selected_compact_mask(
        const void *model_map,
        uint32_t layer,
        const int32_t *selected_ids,
        uint32_t n_total_expert,
    ...
    (void)n_total_expert;
    if (mask == 0) return 1;
    if (!selected_ids || !hip_stream_selected_ensure_stream()) return 0;
    hipError_t err = hipSuccess;
    for (uint32_t i = 0; i < n_selected; i++) {
        ...
        err = hipMemcpyAsync(g_stream_selected_cache.gate +
                              ...
                              g_stream_selected_upload_stream);
        if (err == hipSuccess) {
            err = hipMemcpyAsync(g_stream_selected_cache.up +
                                      (uint64_t)i * gate_expert_bytes,
                                  entry.up,
                                  (size_t)gate_expert_bytes,
                                  hipMemcpyDeviceToDevice,
        ...
        if (err == hipSuccess) {
            err = hipMemcpyAsync(g_stream_selected_cache.down +
    ...
    if (!hip_stream_selected_upload_record_ready()) {
        fprintf(stderr,
...
static int hip_stream_selected_prepare_ptrs(
        const void *model_map,
        uint32_t layer,
        const int32_t *selected_ids,
        uint32_t n_selected,
    ...
    if (!selected_ids ||
        ...
        !hip_stream_selected_ensure_stream()) {
        return 0;
    }
    ...
    memcpy((void *)g_stream_selected_cache.down_ptrs_stage,
           down_ptrs,
           ptr_bytes);
    hipError_t err = hipMemcpyAsync(g_stream_selected_cache.gate_ptrs,
                                      g_stream_selected_cache.gate_ptrs_stage,
                                      ptr_bytes,
                                      hipMemcpyHostToDevice,
                                      g_stream_selected_upload_stream);
    if (err == hipSuccess) {
        err = hipMemcpyAsync(g_stream_selected_cache.up_ptrs,
                              g_stream_selected_cache.up_ptrs_stage,
                              ptr_bytes,
                              hipMemcpyHostToDevice,
                              g_stream_selected_upload_stream);
    ...
    if (err == hipSuccess) {
        err = hipMemcpyAsync(g_stream_selected_cache.down_ptrs,
                              g_stream_selected_cache.down_ptrs_stage,
                              ptr_bytes,
                              hipMemcpyHostToDevice,
                              g_stream_selected_upload_stream);
    ...
    if (err == hipSuccess) {
        upload_record_ok = hip_stream_selected_upload_record_ready();
    }
    if (err != hipSuccess || !upload_record_ok) {
        fprintf(stderr,
...
static int hip_stream_batch_selected_prepare_from_host(
        const void *model_map,
        uint64_t model_size,
        uint32_t layer,
        const int32_t *ids,
    ...
    if (g_stream_batch_selected_pending.active) {
        hip_stream_batch_selected_abort_pending();
    }
    if (g_stream_selected_pending.active) {
        hip_stream_selected_abort_pending();
    }
    g_stream_batch_selected_cache.loaded = 0;

    uint64_t n_ids64 = 0;
    if (!hip_u64_mul_checked(n_tokens, n_selected, &n_ids64) ||
    ...
    if (ok && !hip_stream_batch_selected_ensure_buffers(n_ids64, unique_count)) {
        ok = 0;
    }
    if (ok && !hip_stream_selected_ensure_stream()) ok = 0;
    const int stats_on = hip_stream_cache_stats_on();
    const int layer_stats_on =
        hip_stream_cache_layer_stats_on() &&
        layer < DS4_ROCM_STREAM_CACHE_LAYER_STATS_MAX;
    if (ok && stats_on) {
    ...
    if (ok && layer_stats_on) {
        hip_stream_cache_layer_stats *ls = &g_stream_cache_layer_stats[layer];
        ls->batch_calls++;
    ...
    for (uint32_t u = 0; ok && u < unique_count; u++) {
        ...
        if (idx < 0) {
            ...
            if (use_fd) {
                if (read_job_count + 3u > DS4_ROCM_STREAM_READ_MAX_JOBS) {
                    if (!hip_stream_flush_read_jobs(read_jobs, &read_job_count)) {
                        ok = 0;
                        break;
                    }
                }
    ...
    if (ok && !hip_stream_flush_read_jobs(read_jobs, &read_job_count)) {
        ok = 0;
    }
    if (ok && !use_fd) {
        hipError_t err = hipStreamSynchronize(g_stream_selected_upload_stream);
    ...
    if (ok) {
        ...
        hipError_t err = hipMemcpyAsync(g_stream_batch_selected_cache.selected_ids,
                                          g_stream_batch_selected_cache.selected_stage,
                                          selected_bytes,
                                          hipMemcpyHostToDevice,
                                          g_stream_selected_upload_stream);
        if (err == hipSuccess) {
            err = hipMemcpyAsync(g_stream_batch_selected_cache.pair_missing,
                                  g_stream_batch_selected_cache.pair_missing_stage,
                                  pair_missing_bytes,
                                  hipMemcpyHostToDevice,
                                  g_stream_selected_upload_stream);
        ...
        if (err == hipSuccess) {
            err = hipMemcpyAsync(g_stream_batch_selected_cache.gate_ptrs,
                                  g_stream_batch_selected_cache.gate_ptrs_stage,
                                  ptr_bytes,
                                  hipMemcpyHostToDevice,
                                  g_stream_selected_upload_stream);
        ...
        if (err == hipSuccess) {
            err = hipMemcpyAsync(g_stream_batch_selected_cache.up_ptrs,
                                  g_stream_batch_selected_cache.up_ptrs_stage,
                                  ptr_bytes,
                                  hipMemcpyHostToDevice,
                                  g_stream_selected_upload_stream);
        ...
        if (err == hipSuccess) {
            err = hipMemcpyAsync(g_stream_batch_selected_cache.down_ptrs,
                                  g_stream_batch_selected_cache.down_ptrs_stage,
                                  ptr_bytes,
                                  hipMemcpyHostToDevice,
                                  g_stream_selected_upload_stream);
        ...
        if (err == hipSuccess) {
            err = hipMemcpyAsync(g_stream_batch_selected_cache.resident_gate_ptrs,
                                  g_stream_batch_selected_cache.resident_gate_ptrs_stage,
                                  ptr_bytes,
                                  hipMemcpyHostToDevice,
                                  g_stream_selected_upload_stream);
        ...
        if (err == hipSuccess) {
            err = hipMemcpyAsync(g_stream_batch_selected_cache.resident_up_ptrs,
                                  g_stream_batch_selected_cache.resident_up_ptrs_stage,
                                  ptr_bytes,
                                  hipMemcpyHostToDevice,
                                  g_stream_selected_upload_stream);
        ...
        if (err == hipSuccess) {
            err = hipMemcpyAsync(g_stream_batch_selected_cache.missing_gate_ptrs,
                                  g_stream_batch_selected_cache.missing_gate_ptrs_stage,
                                  ptr_bytes,
                                  hipMemcpyHostToDevice,
                                  g_stream_selected_upload_stream);
        ...
        if (err == hipSuccess) {
            err = hipMemcpyAsync(g_stream_batch_selected_cache.missing_up_ptrs,
                                  g_stream_batch_selected_cache.missing_up_ptrs_stage,
                                  ptr_bytes,
                                  hipMemcpyHostToDevice,
                                  g_stream_selected_upload_stream);
        ...
        if (err == hipSuccess) {
            upload_record_ok =
                hip_stream_batch_selected_upload_record_ready();
        }
        if (err != hipSuccess || !upload_record_ok) {
            fprintf(stderr,
    ...
    if (ok) {
        ...
        *unique_out = unique_count;
        ...
        if (read_job_count != 0) hip_stream_read_jobs_free(read_jobs, read_job_count);
    }
...
static int hip_stream_batch_selected_prepare(
        const void *model_map,
        uint64_t model_size,
        uint32_t layer,
        ...
        uint32_t *unique_out) {
    if (!selected ||
        !hip_tensor_has_elems2(selected, n_tokens, n_selected, sizeof(int32_t))) {
        return 0;
    }
    uint64_t n_ids64 = 0;
    if (!hip_u64_mul_checked(n_tokens, n_selected, &n_ids64) ||
    ...
    const int copy_ok = hip_ok(hipMemcpy(ids,
                                           selected->ptr,
                                           (size_t)n_ids64 * sizeof(ids[0]),
                                           hipMemcpyDeviceToHost),
                                "streaming batch selected ids copy");
    const int ok = copy_ok &&
        hip_stream_batch_selected_prepare_from_host(model_map,
                                                     model_size,
                                                     layer,
                                                     ids,
                                                     n_tokens,
...
static int hip_stream_layer_expert_cache_prepare_batch(
        const void *model_map,
        uint32_t layer,
        const ds4_gpu_tensor *selected,
        uint32_t n_tokens,
    ...
    if (!hip_stream_layer_expert_cache_apply(model_map,
                                              layer,
                                              n_total_expert,
                                              gate_offset,
                                              up_offset,
    ...
    if (ok) {
        ...
        if (err == hipSuccess) err = hipStreamSynchronize(g_stream_selected_upload_stream);
        if (err != hipSuccess) {
            fprintf(stderr,
...
static int hip_stream_layer_expert_cache_seed_selected(
        const void *model_map,
        uint64_t model_size,
        uint32_t layer,
        const ds4_gpu_tensor *selected,
    ...
    if (!hip_stream_layer_expert_cache_apply(model_map,
                                              ...
                                              &layer_up,
                                              &layer_down)) {
        if (!hip_model_range_is_cached(model_map, gate_offset, gate_bytes) ||
            !hip_model_range_is_cached(model_map, up_offset, gate_bytes) ||
            !hip_model_range_is_cached(model_map, down_offset, down_bytes)) {
            return 0;
        }
        layer_gate = hip_model_range_ptr(model_map,
                                          gate_offset,
                                          gate_bytes,
                                          "streaming full-layer seed gate");
        layer_up = hip_model_range_ptr(model_map,
                                        up_offset,
                                        gate_bytes,
                                        "streaming full-layer seed up");
        layer_down = hip_model_range_ptr(model_map,
                                          down_offset,
                                          down_bytes,
    ...
    if (ok && hip_stream_cache_stats_on()) {
        g_stream_cache_stats.seed_calls++;
        g_stream_cache_stats.seed_unique += unique_count;
    }

    if (ok && unique_count != 0 && !hip_stream_selected_ensure_stream()) {
        ok = 0;
    }
    ...
    if (!ok) hip_stream_resident_cache_release();
    free(ids_heap);
    free(unique_heap);
    return ok;
}

static int hip_stream_selected_load(
        const void *model_map,
        uint64_t model_size,
        uint32_t layer,
        const int32_t *selected_ids,
    ...
    uint64_t gate_bytes = 0;
    uint64_t down_bytes = 0;
    if (!hip_u64_mul_checked(n_selected, gate_expert_bytes, &gate_bytes) ||
        !hip_u64_mul_checked(n_selected, down_expert_bytes, &down_bytes)) {
        return 0;
    }
    if (!hip_stream_selected_reuse_wait("streaming selected cache reuse")) {
        return 0;
    }
    if (!hip_stream_selected_ensure_buffers(gate_bytes, down_bytes)) return 0;
    if (!hip_stream_selected_ensure_stream()) return 0;
    hip_stream_selected_cache_header(model_map,
                                      layer,
                                      n_total_expert,
                                      n_selected,
                                      selected_ids,
    ...
    if (resident_mask != 0 && missing_mask == 0) {
        ...
        if (!hip_stream_selected_prepare_ptrs(model_map,
                                               layer,
                                               selected_ids,
                                               n_selected,
                                               gate_offset,
    ...
    if (use_fd && read_job_count != 0) {
        ...
        memcpy(g_stream_selected_pending.read_jobs,
               read_jobs,
               (size_t)read_job_count * sizeof(read_jobs[0]));
        if (!hip_stream_read_jobs_start(g_stream_selected_pending.read_jobs,
                                         read_job_count)) {
            memset(&g_stream_selected_pending, 0, sizeof(g_stream_selected_pending));
            hip_stream_read_jobs_free(read_jobs, read_job_count);
            hip_stream_resident_cache_release();
    ...
    if (resident_mask != 0 &&
        !hip_stream_selected_compact_mask(model_map,
                                           layer,
                                           selected_ids,
                                           n_total_expert,
                                           n_selected,
    ...
    if (read_job_count != 0) {
        if (!hip_stream_read_jobs_parallel(read_jobs, read_job_count) ||
            !hip_stream_selected_upload_read_jobs(read_jobs, read_job_count)) {
            hip_stream_read_jobs_free(read_jobs, read_job_count);
            hip_stream_resident_cache_release();
            return 0;
...

    {
        ...
        if (!hip_stream_selected_compact_mask(model_map,
                                               layer,
                                               selected_ids,
                                               n_total_expert,
                                               n_selected,
...
static int hip_stream_selected_pending_matches(
        const void *model_map,
        uint32_t layer,
        uint32_t n_total_expert,
        uint32_t n_selected,
...
static int hip_stream_selected_finish_pending_missing(uint32_t compact_mask);

static int hip_stream_selected_apply_split(
        const void *model_map,
        uint32_t layer,
        uint32_t n_total_expert,
        uint32_t n_selected,
    ...
    if (!g_ssd_streaming_mode ||
        ...
        !resident_mask ||
        !missing_mask ||
        !hip_stream_selected_pending_matches(model_map,
                                              layer,
                                              n_total_expert,
                                              n_selected,
                                              gate_expert_bytes,
...
static int hip_stream_selected_finish_pending_missing(uint32_t compact_mask) {
    ...
    const uint32_t read_job_count = g_stream_selected_pending.read_job_count;
    if (!hip_stream_read_jobs_wait(g_stream_selected_pending.read_jobs,
                                    read_job_count) ||
        !hip_stream_selected_upload_read_jobs(g_stream_selected_pending.read_jobs,
                                              read_job_count)) {
        hip_stream_read_jobs_free(g_stream_selected_pending.read_jobs,
                                   read_job_count);
        memset(&g_stream_selected_pending, 0,
    ...
    hip_stream_read_jobs_free(g_stream_selected_pending.read_jobs,
                               read_job_count);
    if (compact_mask != 0 &&
        !hip_stream_selected_compact_mask(
...
static int hip_stream_selected_apply(
        const void *model_map,
        uint32_t layer,
        uint32_t n_total_expert,
        uint32_t n_selected,
    ...
    if (g_ssd_streaming_mode &&
        ...
        if (!hip_stream_selected_finish_pending_missing(compact_mask)) {
            return 0;
        }
    }
    if (!g_ssd_streaming_mode ||
...
static const char *hip_model_ptr(const void *model_map, uint64_t offset) {
    const char *owned = hip_model_image_ptr(model_map, offset);
...
static const char *hip_model_range_copy_uncached(
        const void *model_map,
        uint64_t offset,
        uint64_t bytes,
        const char *what) {
    ...
    err = hipMemcpy(dev, src, (size_t)bytes, hipMemcpyHostToDevice);
    if (err != hipSuccess) {
...
static const char *hip_model_range_ptr(const void *model_map, uint64_t offset, uint64_t bytes, const char *what) {
    if (bytes == 0) return hip_model_ptr(model_map, offset);
    const char *image_ptr =
        hip_model_image_range_ptr(model_map, offset, bytes);
    ...
    const char *fd_ptr = hip_model_range_ptr_from_fd(model_map, offset, bytes, what);
    ...
    if (model_map != g_model_host_base) {
        return hip_model_range_copy_uncached(model_map, offset, bytes, what);
    }
    ...
    if (g_model_range_mapping_supported && model_map == g_model_host_base) {
        ...
        if (err == hipSuccess) {
            ...
            if (err == hipErrorNotSupported || err == hipErrorInvalidValue) g_model_range_mapping_supported = 0;
            (void)hipGetLastError();
        }
    }
...
static int hip_model_range_is_cached(const void *model_map, uint64_t offset, uint64_t bytes) {
    if (bytes == 0) return 1;
    if (hip_model_image_range_ptr(model_map, offset, bytes)) return 1;
...
static uint32_t hip_rows_per_block_env_or_default(const char *name, uint32_t def) {
    ...
    return hip_rows_per_block_or_default((uint32_t)v, def);
}
    ...
    if (!g_rocm_cfg.initialized) {
        ...
        g_rocm_cfg.q8_prequant_decode =
            ...
            (dsv4_prequant_env == NULL ||
             hip_env_present(dsv4_prequant_env));
        g_rocm_cfg.disable_splitk_attn_out_low = !g_quality_mode;
        g_rocm_cfg.disable_shared_gate_up_fused_w32 = !g_quality_mode;
        g_rocm_cfg.attention_output_hipblas_all = !g_quality_mode;
        g_rocm_cfg.shared_down_hipblas = !g_quality_mode;
        ...
        g_rocm_cfg.glm_grouped_value_project =
            ...
            hip_env_present(glm_grouped_value_project_env);
        ...
        g_rocm_cfg.glm_grouped_qk_low =
            ...
            hip_env_present(glm_grouped_qk_low_env);
        ...
        g_rocm_cfg.q8_decode_sharedx_64k =
            ...
            hip_env_present(q8_decode_sharedx_64k_env);
        const int graph_dump_requested =
            hip_env_present(getenv("DS4_ROCM_GRAPH_DUMP_PREFIX")) ||
            hip_env_present(getenv("DS4_METAL_GRAPH_DUMP_PREFIX"));
        ...
        const int graph_dump_noninvasive =
            hip_env_present(getenv("DS4_ROCM_GRAPH_DUMP_NONINVASIVE"));
        ...
        g_rocm_cfg.moe_decode_rpb =
            hip_rows_per_block_env_or_default("DS4_ROCM_MOE_DECODE_RPB",
                                               g_quality_mode ? 8u :
                                               (g_ssd_streaming_mode ? 2u : 1u));
        g_rocm_cfg.moe_decode_gate_rpb =
            hip_rows_per_block_env_or_default("DS4_ROCM_MOE_DECODE_GATE_RPB",
                                               (!g_quality_mode &&
                                                g_ssd_streaming_mode &&
                                                !moe_decode_rpb_env_present) ?
                                                    1u : g_rocm_cfg.moe_decode_rpb);
        g_rocm_cfg.moe_decode_down_rpb =
            hip_rows_per_block_env_or_default("DS4_ROCM_MOE_DECODE_DOWN_RPB",
                                               (!g_quality_mode &&
                                                g_ssd_streaming_mode &&
                                                !moe_decode_rpb_env_present) ?
...
static bool hip_q8_prequant_decode_enabled(void) {
    return !g_glm_model && hip_runtime_config()->q8_prequant_decode;
}
...
static uint64_t hip_q8_f16_cache_reserve_bytes(uint64_t total_bytes) {
    if (g_ssd_streaming_mode) {
        return hip_stream_resident_free_reserve_bytes();
    }
     ...
     * device memory free for hipBLAS workspaces, transient graph buffers, and
...
static int hip_q8_f16_cache_allowed(const char *label, uint64_t in_dim, uint64_t out_dim) {
    ...
    if (getenv("DS4_ROCM_NO_Q8_F16_CACHE") != NULL) return 0;
    if (!label) return 0;
...
static int hip_q8_f16_preload_allowed(const char *label, uint64_t in_dim, uint64_t out_dim) {
    if (hip_q8_label_is_attention_output(label) &&
        !hip_runtime_config()->attention_output_hipblas_all) {
        return 0;
    }
    return hip_q8_f16_cache_allowed(label, in_dim, out_dim);
}
...
static uint32_t hip_prefill_warmup_tokens(void) {
...
static void hip_q8_f16_warmup_attention_output_a_gemm(const __half *out_a_f16,
                                                       uint64_t group_dim,
                                                       uint64_t rank,
                                                       uint32_t n_groups) {
    static int warmed = 0;
    if (warmed || !g_hipblas_ready || !out_a_f16 || group_dim == 0 || rank == 0 || n_groups == 0) return;
    const ds4_rocm_runtime_config *cfg = hip_runtime_config();
    if (!cfg->attention_output_hipblas_all) return;
    warmed = 1;
    const uint32_t n_tok = hip_prefill_warmup_tokens();
    ...
    void *tmp = hip_tmp_alloc(low_h_off + low_h_count * sizeof(__half), "attention output a warmup");
    if (!tmp) return;
    __half *heads_h = (__half *)tmp;
    __half *low_h = (__half *)((char *)tmp + low_h_off);
    if (hipMemset(heads_h, 0, (size_t)heads_h_bytes) != hipSuccess) return;
    ...
    if (st == HIPBLAS_STATUS_SUCCESS) (void)hipDeviceSynchronize();
}

static void hip_q8_f16_warmup_attention_output_b_gemm(const __half *out_b_f16_t,
                                                       uint64_t low_dim,
                                                       uint64_t out_dim) {
    static int warmed = 0;
    if (warmed || !g_hipblas_ready || !out_b_f16_t || low_dim == 0 || out_dim == 0) return;
    if (!hip_runtime_config()->attention_output_hipblas_all) return;
    warmed = 1;
    const uint32_t n_tok = hip_prefill_warmup_tokens();
    ...
    void *tmp = hip_tmp_alloc(out_off + out_count * sizeof(float), "attention output b warmup");
    if (!tmp) return;
    __half *low_h = (__half *)tmp;
    float *out = (float *)((char *)tmp + out_off);
    if (hipMemset(low_h, 0, (size_t)low_h_bytes) != hipSuccess) return;
    const float alpha = 1.0f;
    const float beta = 0.0f;
...
static int hip_model_stage_pool_alloc(uint64_t bytes) {
    ...
    for (size_t i = 0; i < 4; i++) {
        ...
        if (g_model_stage_raw[i]) {
            (void)hipHostFree(g_model_stage_raw[i]);
    ...
    for (size_t i = 0; i < 4; i++) {
        hipError_t err = hipHostMalloc(&g_model_stage_raw[i], (size_t)alloc_bytes);
        if (err != hipSuccess) {
...
static int hip_stream_model_cache_prepare_memory(
        uint64_t request_bytes,
        const char *what) {
...
static char *hip_model_arena_alloc(uint64_t bytes, const char *what) {
    ...
    if (err != hipSuccess) {
        ...
        while (fallback >= aligned) {
            err = hipMalloc(&dev, (size_t)fallback);
            if (err == hipSuccess) break;
            (void)hipGetLastError();
            fallback /= 2u;
        }
        if (err != hipSuccess) {
            err = hipMalloc(&dev, (size_t)aligned);
            if (err != hipSuccess) {
                fprintf(stderr,
...
static const char *hip_model_range_ptr_from_fd(
    ...
    if (g_model_range_bytes > limit || bytes > limit - g_model_range_bytes) {
        ...
        return hip_model_ptr(model_map, offset);
    }
...
static int hip_model_copy_chunked(const void *model_map, uint64_t model_size, uint64_t map_offset, uint64_t map_size) {
    if (!model_map || model_size == 0 || map_offset > model_size || map_size > model_size - map_offset) return 0;
    if (map_size == 0) return 0;
    if (hip_model_image_range_ptr(model_map, map_offset, map_size)) return 1;

    void *dev = NULL;
    const double t0 = hip_wall_sec();
    hipError_t err = hipMalloc(&dev, (size_t)map_size);
    if (err != hipSuccess) {
    ...
    if (!hip_model_stage_pool_alloc(stage_bytes)) {
        (void)hipFree(dev);
        return 0;
    }
    ...
    while (copied < map_size) {
        ...
        err = hipMemcpyAsync((char *)dev + copied, payload, (size_t)n,
                              hipMemcpyHostToDevice, g_model_upload_stream);
        if (err != hipSuccess) {
        ...
        hip_model_drop_file_pages(map_offset + copied, n);
        hip_model_discard_source_pages(model_map, model_size,
                                        map_offset + copied, n);
        copied += n;
        chunk_idx++;
        hip_model_load_progress_note(copied);
    }
    err = hipStreamSynchronize(g_model_upload_stream);
    if (err != hipSuccess) {
...
static void hip_model_range_release_ranges_only(void) {
    for (const hip_model_range &r : g_model_ranges) {
        if (r.host_registered && r.registered_base) {
            (void)hipHostUnregister(r.registered_base);
...
static void hip_model_range_release_all(void) {
    hip_model_range_release_ranges_only();
    g_stream_selected_cache.loaded = 0;
    hip_stream_resident_cache_release();
...
extern "C" int ds4_gpu_init(void) {
    ...
    if (!g_hipblas_ready) {
        ...
        const hipblasMath_t math_mode = g_quality_mode ? HIPBLAS_DEFAULT_MATH : HIPBLAS_TF32_TENSOR_OP_MATH;
        (void)hipblasSetMathMode(g_hipblas, math_mode);
        g_hipblas_ready = 1;
    }
#ifdef __HIP_PLATFORM_AMD__
...
extern "C" void ds4_gpu_cleanup(void) {
    (void)hipDeviceSynchronize();
    hip_stream_cache_stats_print("cleanup");
    ...
    hip_q8_f16_cache_release_all();
    hip_stream_selected_cache_release();
...
extern "C" int ds4_gpu_tensor_copy(ds4_gpu_tensor *dst, uint64_t dst_offset,
    ...
    if (bytes == 0) return 1;
    return hip_ok(hipMemcpyAsync((char *)dst->ptr + dst_offset,
...
extern "C" int ds4_gpu_signal_selected_readback_ready(uint64_t *event_value) {
    ...
    *event_value = 0;
    if (!g_selected_readback_event) {
        hipError_t err =
            hipEventCreateWithFlags(&g_selected_readback_event,
                                     hipEventDisableTiming);
        if (err != hipSuccess) {
            fprintf(stderr,
    ...
    hipError_t err = hipEventRecord(g_selected_readback_event, 0);
    if (err != hipSuccess) {
        fprintf(stderr,
...
extern "C" int ds4_gpu_commit_and_wait_selected_readback(uint64_t event_value, const char *label) {
    ...
    return hip_ok(hipEventSynchronize(g_selected_readback_event),
...
extern "C" int ds4_gpu_set_model_map(const void *model_map, uint64_t model_size) {
    ...
    g_model_host_base = model_map;
    g_model_device_base = hip_model_image_owned(model_map) ?
                          hip_model_image_ptr(model_map, 0) :
                          (const char *)model_map;
    g_model_registered_size = model_size;
    g_model_device_owned = hip_model_image_owned(model_map);
    g_model_range_mapping_supported = 1;
    g_model_cache_full = 0;
...
extern "C" int ds4_gpu_set_model_map_range(const void *model_map, uint64_t model_size, uint64_t map_offset, uint64_t map_size, uint64_t max_tensor_bytes) {
    ...
    if (g_ssd_streaming_mode) {
        ...
        if (!hip_stream_model_cache_prepare_memory(map_size,
                                                    "streaming model range")) {
            return 0;
        }
        if (!hip_model_range_ptr(model_map, map_offset, map_size, "stream_range")) return 0;
        return hip_model_range_is_cached(model_map, map_offset, map_size);
    }
    /*
...
extern "C" int ds4_gpu_set_model_map_spans(
    ...
    if (g_ssd_streaming_mode) {
        ...
        for (uint32_t i = 0; i < count; i++) {
            if (!hip_u64_add_checked(request_bytes, sizes[i], &request_bytes)) {
                fprintf(stderr,
        ...
        if (!hip_stream_model_cache_prepare_memory(request_bytes,
                                                    "streaming model spans")) {
            return 0;
        }
        for (uint32_t i = 0; i < count; i++) {
            if (!hip_model_range_ptr(model_map, offsets[i], sizes[i], "stream_span")) return 0;
            if (!hip_model_range_is_cached(model_map, offsets[i], sizes[i])) return 0;
        }
        return 1;
    }
    ...
    if (bbox <= span_bytes + span_bytes / 10u) {
        return hip_model_copy_chunked(model_map, model_size,
                                       min_offset, bbox);
    }
    ...
    for (uint32_t i = 1; i <= count; i++) {
        ...
        if (!hip_model_copy_chunked(model_map, model_size,
...
extern "C" int ds4_gpu_cache_q8_f16_range(const void *model_map, uint64_t model_size, uint64_t offset, uint64_t bytes, uint64_t in_dim, uint64_t out_dim, const char *label) {
    ...
    if (!hip_q8_f16_preload_allowed(cache_label, in_dim, out_dim)) return 1;
    const int preload_transposed_b = !g_quality_mode &&
                                     strstr(cache_label, "attn_output_b") != NULL;
    if (preload_transposed_b) {
        const __half *f16_t = hip_q8_f16_transpose_ptr(model_map, offset, bytes, in_dim, out_dim, cache_label);
        if (f16_t) {
            if (strstr(cache_label, "attn_output_b") != NULL && in_dim == 8192u && out_dim == 4096u) {
                hip_q8_f16_warmup_attention_output_b_gemm(f16_t, in_dim, out_dim);
            }
            return 1;
        }
    } else {
        const __half *f16 = hip_q8_f16_ptr(model_map, offset, bytes, in_dim, out_dim, cache_label);
        if (f16) {
            if (strstr(cache_label, "attn_output_a") != NULL && in_dim == 4096u && out_dim == 8192u) {
                hip_q8_f16_warmup_attention_output_a_gemm(f16, in_dim, 1024u, 8u);
            }
            return 1;
        }
    }
...
extern "C" void ds4_gpu_print_memory_report(const char *label) {
    ...
    if (err != hipSuccess) {
        fprintf(stderr, DS4_GPU_LOG_PREFIX "memory %s: query failed: %s\n",
                label ? label : "", hipGetErrorString(err));
        (void)hipGetLastError();
        return;
    }
    const uint64_t used_b = (uint64_t)total_b - (uint64_t)free_b;
    const char *placement = hip_model_image_bytes() ? "device_copy" : "mapped/range_cache";
    fprintf(stderr,
            ...
            placement,
            (double)hip_model_image_bytes() / 1073741824.0,
            (double)g_model_range_bytes / 1073741824.0,
            (double)g_q8_f16_bytes / 1073741824.0,
            (double)g_hip_tmp_bytes / 1073741824.0);
    fprintf(stderr, "\n");
}
