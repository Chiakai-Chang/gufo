extern "C" int ds4_gpu_embed_token_hc_tensor(ds4_gpu_tensor *out_hc, const void *model_map, uint64_t model_size, uint64_t weight_offset, uint32_t n_vocab, uint32_t token, uint32_t n_embd, uint32_t n_hc) {
    ...
    if (!hip_u64_mul3_checked(n_vocab, n_embd, sizeof(uint16_t), &weight_bytes) ||
        !hip_model_range_fits(model_size, weight_offset, weight_bytes) ||
        !hip_u64_mul3_checked(n_hc, n_embd, sizeof(float), &out_bytes) ||
        !hip_tensor_has_bytes(out_hc, out_bytes)) return 0;
    const char *wptr = hip_model_range_ptr(model_map, weight_offset, weight_bytes, "token_embd");
    if (!wptr) return 0;
    ...
    return hip_ok(hipGetLastError(), "embed token launch");
}
...
extern "C" int ds4_gpu_embed_tokens_hc_tensor(
    ...
    if (!hip_u64_mul3_checked(n_vocab, n_embd, sizeof(uint16_t), &weight_bytes) ||
        !hip_model_range_fits(model_size, weight_offset, weight_bytes) ||
        !hip_tensor_has_i32(tokens_t, n_tokens) ||
        !hip_u64_mul_checked(n_tokens, n_hc, &n) ||
        !hip_u64_mul_checked(n, n_embd, &n) ||
        !hip_tensor_has_f32(out_hc, n)) {
        return 0;
    }
    const char *wptr = hip_model_range_ptr(model_map, weight_offset, weight_bytes, "token_embd");
    if (!wptr) return 0;
    ...
    return hip_ok(hipGetLastError(), "embed tokens launch");
}
...
extern "C" int ds4_gpu_embed_token_hc_q8_0_tensor(
    ...
    if (!hip_u64_mul_checked(blocks, 34u, &row_bytes) ||
        !hip_u64_mul_checked(n_vocab, row_bytes, &weight_bytes) ||
        !hip_model_range_fits(model_size, weight_offset, weight_bytes) ||
        !hip_u64_mul3_checked(n_embd, n_hc, sizeof(float), &out_bytes) ||
        !hip_tensor_has_bytes(out_hc, out_bytes)) {
        return 0;
    }
    const char *wptr = hip_model_range_ptr(model_map, weight_offset, weight_bytes, "token_embd_q8_0");
    if (!wptr) return 0;
    ...
    embed_token_hc_q8_0_kernel<<<(n + 255u) / 256u, 256>>>(
            ...
            n_embd,
            n_hc);
    return hip_ok(hipGetLastError(), "embed token q8_0 launch");
}
...
extern "C" int ds4_gpu_embed_tokens_hc_q8_0_tensor(
    ...
    embed_tokens_hc_q8_0_kernel<<<(n + 255u) / 256u, 256>>>(
            ...
            n_embd,
            n_hc);
    return hip_ok(hipGetLastError(), "embed tokens q8_0 launch");
}
