 ...
 *
 * The parser does not link ROCM headers directly. On ROCM builds the
 * "auto" path is gated and dispatches to ds4_gpu_args_probe_auto_hip,
 * which is defined in ds4_rocm.hip.cpp. On Metal/CPU builds, "auto" returns
 * an error.
 */
...
/* Single canonical parser. Populates *out and *out_skip_hip on
 * success; returns 0 on success, nonzero on parse error (errbuf
 * populated). Caller zeroes out and out_skip_hip before calling.
 *
 *   vram_arg may be:
 *     NULL       -- flag unset
 *     "0"        -- skip ROCM entirely; sets *out_skip_hip = true
 *     "auto"     -- query free VRAM via ROCM (ROCM build only)
 *     "N[,N...]" -- explicit GB budgets per device
 *
 *   devices_arg may be:
 *     NULL       -- 0..n-1 of visible devices (or 0..vram_count-1)
 *     "N[,N...]" -- explicit ROCM device indices
 *
 * Constraints:
...
/* ROCM-only auto probe. Defined in ds4_rocm.hip.cpp under the same gate.
...
int ds4_gpu_args_probe_auto_hip(const int      *device_filter,
                                  int             filter_len,
                                  ds4_gpu_config *out,
