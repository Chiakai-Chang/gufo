...
std::unique_ptr<BpeTokenizer> BpeTokenizer::CreateFromVocabulary(
...
void BpeTokenizer::InitializeByteTokens(bool eager_decoded_tokens) {
...
std::vector<TokenId> BpeTokenizer::BpeMergeChunk(std::string_view chunk) const {
  if (chunk.empty()) {
    return {};
  }
...
std::string_view BpeTokenizer::DecodeToken(TokenId token_id) const noexcept {
...
std::string BpeTokenizer::DecodeTokenCopy(TokenId token_id) const {
  if (token_id >= id_to_token_.size()) {
    return {};
  }
...
std::optional<TokenId> BpeTokenizer::FindSpecialToken(
...
bool BpeTokenizer::IsSpecialToken(TokenId id) const noexcept {
