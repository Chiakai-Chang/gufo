...
struct InferenceBackend::Impl {
  ...
  struct State {
    ...
    std::shared_ptr<GpuSessionPool> sessions;
    std::string model_id;
  };
  ...
  Result Generate(std::shared_ptr<const State> current,
    ...
    auto lease = current->sessions->Acquire(is_cancelled);
    if (!lease) {
      result.cancelled = true;
      EmitRequestMetrics(result, "cancelled");
      return result;
    ...
    result.text = current->model->GetTokenizer().Decode(result.tokens);
...
bool InferenceBackend::load(std::shared_ptr<const hip::QwenGpuModel> model,
  ...
  try {
    ...
    new_state->model = std::move(model);
    new_state->sessions = std::make_shared<GpuSessionPool>(
        new_state->model, max_context, session_count);
    new_state->model_id = new_state->model->GetConfig().model_name;
    {
...
InferenceBackend::Result InferenceBackend::complete(
  ...
  const auto prompt_tokens = state->model->GetTokenizer().Encode(prompt);
  return impl_->Generate(state, prompt_tokens, request_start, max_tokens,
...
InferenceBackend::Result InferenceBackend::chat(
  ...
  const auto prompt_tokens = tokenization::QwenChatTemplate::RenderAndTokenize(
      state->model->GetTokenizer(), messages);
...
std::size_t InferenceBackend::count_tokens(std::string_view text) const {
  ...
  return state->model->GetTokenizer().Encode(text).size();
#else
  (void)text;
  return 0;
#endif
