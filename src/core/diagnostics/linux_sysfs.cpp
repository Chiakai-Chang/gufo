...
std::vector<SysfsNpuDevice> LinuxSysfs::QueryAccelNpuDevices() const {
  std::vector<SysfsNpuDevice> devices;
  const auto accel_path = sys_root_ / "class" / "accel";

  std::error_code ec;
  if (!std::filesystem::exists(accel_path, ec)) {
    return devices;
  }

  for (const auto& entry :
       std::filesystem::directory_iterator(accel_path, ec)) {
    if (ec) {
      break;
    }
    const std::string filename = entry.path().filename().string();
    if (!filename.starts_with("accel")) {
      continue;
    }
    ...
    SysfsNpuDevice npu;
    npu.accel_name = filename;
    npu.vendor_id = ReadFile(dev_dir / "vendor").value_or("");
    npu.device_id = ReadFile(dev_dir / "device").value_or("");
    ...
    if (uevent_content) {
      ...
      while (std::getline(stream, line)) {
        if (line.starts_with("DRIVER=")) {
          npu.driver = line.substr(7);
        } else if (line.starts_with("PCI_ID=")) {
          npu.pci_id = line.substr(7);
        } else if (line.starts_with("PCI_SLOT_NAME=")) {
          npu.pci_slot = line.substr(14);
        }
      }
    }

    devices.push_back(std::move(npu));
  }
